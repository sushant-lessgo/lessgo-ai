# writerFlownTemplate.md — Writer Vertical + Granth Template

Track plan doc (single doc for this track, like meridianPlan.md / nsoPlan.md).
**Visual source of truth: `WRDirection1Granth.html` (repo root).** Design brief: `writerDesignBrief.md`.
...
## 2. Design spec — Granth (from `WRDirection1Granth.html`)
...
**Clone base: `lumen`** (closest precedent: bespoke containment, contained editable component `LumenEditable.tsx`, external-link popover `LinkTargetPopover.tsx`, per-block shared `styles.ts`, `registration.test.ts`). Follow `newTemplate.md` for the full checklist.
...
**GATE A:** published page vs `WRDirection1Granth.html` side-by-side parity (desktop + 360px mobile) + user approval.
...
- Manual (TESTING.md style): editor↔published parity per section · 360px mobile · palette/variant toggles · optional-section drop (no writing, no praise) · 2-book and 8-book shelf · long names (e.g. "केशव नारायण 'अरण्य'") · all outlinks `_blank noopener` · Lighthouse on published page (fonts subset properly; LCP = hero name)
