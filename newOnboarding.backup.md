...
### Overview
The flow from one-liner to generated page, optimized for minimal user input while gathering necessary context.
...
1. One-liner
       ↓
2. Understanding + Features (always show)
       ↓
3. Landing Action + Offer
       ↓
4. Asset Availability (simple checkboxes)
       ↓
5. Research (IVOC)
       ↓
6. Strategy + Section Selection (outputs: vibe, oneReader, oneIdea, sections)
       ↓
7. Design (Background, Accent, Font) ← derived from vibe, automatic
       ↓
8. UIBlock Selection (AI + user questions if needed)
       ↓
9. Copy Generation (single call, all sections)
       ↓
10. Page Assembly + Guidance
```

**API Calls Summary:**
| Step | Calls | Model |
|------|-------|-------|
| 2. Understanding + Features | 1 | Mid-tier |
| 5. Research (IVOC) | 1 | Tavily + Mid-tier |
| 6. Strategy + Sections | 1 | Mid-tier |
| 7. Design | 0 | — (mapping) |
| 8. UIBlock selection | 1-2 | Mid-tier |
| 9. Copy generation | 1 | Mid-tier |
| **Total** | **5-6 calls** | |
...
|------|-------------|-----------|
| 1 | Provides one-liner | Yes |
| 2 | Reviews understanding + features | Yes (always show) |
| 3 | Selects landing action + offer | Yes |
| 4 | Confirms asset availability | Yes |
| 8 | Answers UIBlock questions (if any) | Sometimes |
| 10 | Reviews page, verifies flagged content | Yes |

**Total: 4-5 interactions before generation**
...
### Old Approach
- 150 LOC rule-based
- Category keywords → 3 buckets (technical/professional/friendly) → random

### New Approach
- **AI infers vibe from one-liner** (single AI call)
- **5 vibe options:** Dark Tech, Light Trust, Warm Friendly, Bold Energy, Calm Minimal
- Random background from vibe pool
- No user confirmation - they can change in editor

### Derived Colors (No change needed)
- **Secondary:** Derived from primary's baseColor (rgba with 50 shade at 70% opacity)
- **Neutral:** Always white
- **Divider:** Derived from baseColor at 50% opacity
...
### Old Approach
- 250 LOC, 7 variables → tags → scoring
- Many ties, unpredictable

### New Approach
- **Reuse vibe from background selection**
- Map vibe → accent energy → filter by baseColor + energy → random
- Same baseColor produces different accents for different vibes
- One AI call for vibe, two decisions (background + accent)

---

## 3. Optional Element Selection

### Old Approach
- Complex rules to decide which optional elements to include per UIBlock
- Hundreds of LOC

### New Approach
- **REMOVED as separate system**
- Three element types with different handling:

| Element Type | Decision Logic |
|--------------|----------------|
| **AI-generated** (text) | AI decides → returns value or NULL |
| **User-preferred** (images, manual inputs) | Default ON/OFF config per element |

### For AI-generated elements:
- AI returns value → include
- AI returns NULL → exclude
- No default config needed - AI is smart

### For User-preferred elements (images, manual inputs):
- **Default ON:** Placeholder shown, user replaces or deletes
- **Default OFF:** Hidden, user adds via "Add Element" if needed
- Simple static default per element in UIBlock definition

### Philosophy
- Include sensible defaults (e.g., 3 trust items, not 6)
- User can always add more via "Add Element"
- User can delete what they don't want
- Guide user: "Upload your hero image", "Add your pricing"

---

## 4. Card Count / Array Items

### Old Approach
- Strategy phase asks "how many cards?"
- AI guesses number before generating content
- Two-phase coordination complexity

### New Approach
- **Remove card count from strategy phase**
- **AI generates content in array format**
- Card count = array.length (output, not input)
- AI generates what's meaningful for the product
- Soft guidance in prompt: "generate 3-6 features" as range, not fixed number

### Benefits
- Quality > Quantity (4 compelling features > 8 generic ones)
- No forced slot-filling
- Natural output
...
### Current taxonomy.ts Contents
- Market categories (21)
- Market subcategories (100+)
- Landing page goals (12) → **Simplified to 6**
- Startup stages (~15) → **REMOVED**
- Target audiences (50+)
- Pricing models (9)
- Awareness levels (5)
...
### What to KEEP
- **Landing page goals (6)** - waitlist, signup, free-trial, buy, demo, download
- **Pricing models + commitment options + modifiers** - affects Pricing UIBlock

### What to REMOVE
- **Market subcategories** - never used functionally
- **Startup stages** - redundant, asset availability + landing action sufficient

### What to SIMPLIFY
- **Market categories** (21 → AI infers or reduce to 5)
- **Target audiences** - AI infers from one-liner
- **Tone profiles** - merge with vibe concept

### What AI INFERS (no need to store/ask)
- Awareness level
...
### Old Approach
- 5 tone profiles → 3 font options each = 15 combinations
- References fonts not self-hosted (Google Fonts dependency)
- ~115 LOC

### New Approach
- **Vibe → deterministic font pair**
- Only 4 pre-downloaded fonts used
- ~20 LOC
...
### Pre-Downloaded Fonts
- Inter (sans-serif, neutral)
- Sora (sans-serif, modern/tech)
- DM Sans (sans-serif, friendly)
- Playfair Display (serif, elegant)

---

## 7. Placeholder Images Strategy
...
## 11. UIBlock Selection (Locked for MVP)

### Principles
1. **Distinct use case** - each UIBlock serves different purpose
2. **Different inputs** - images vs text vs metrics
3. **Different vibe/mental model** - emotional vs analytical vs technical
4. **Visual variety** - keeps generated pages looking fresh
5. **Random tiebreaker** - if rules match equally, pick random
6. **Section fit** - UIBlock belongs in this section

### BeforeAfter (7 UIBlocks)

| UIBlock | Keep/Move | Selection Signal |
|---------|-----------|------------------|
| SideBySideBlock | ✅ KEEP | Default text-based, simple A→B |
| StackedTextVisual | ✅ KEEP | Vertical layout variety |
| SplitCard | ✅ KEEP | Premium vibe, needs images |
| BeforeAfterSlider | ✅ KEEP | Interactive, needs before/after images |
| StatComparison | ✅ KEEP | Metrics-focused (hasCompellingMetrics) |
| TextListTransformation | ✅ KEEP | Many pain points (4+) |
| PersonaJourney | ✅ KEEP | Persona transformation story |
| VisualStoryline | → HowItWorks | Timeline = process |

**Selection Logic:**
```
hasCompellingMetrics → StatComparison
hasBeforeAfterImages → RANDOM(BeforeAfterSlider, SplitCard)
hasPersonas → PersonaJourney
else → RANDOM(SideBySideBlock, StackedTextVisual, TextListTransformation)
```
...
| UIBlock | Keep/Move | Selection Signal |
|---------|-----------|------------------|
| IconGrid | ✅ KEEP | Default, no images needed |
| MiniCards | ✅ KEEP | High feature count (8+) |
| MetricTiles | ✅ KEEP | Feature metrics/ROI numbers |
| Carousel | ✅ KEEP | Scrollable, needs feature images |
| SplitAlternating | ✅ KEEP | Alternating layout, needs images |
| Tabbed | ✅ KEEP | Categorizable features |
| Timeline | → HowItWorks | Sequential = process |
| FeatureTestimonial | ❌ REMOVE | Too niche, rarely have testimonials per feature |

**Selection Logic:**
```
hasFeatureMetrics → MetricTiles
hasFeatureImages → RANDOM(SplitAlternating, Carousel)
hasFeatureCategories → Tabbed
else → RANDOM(IconGrid, MiniCards)
```
...
| UIBlock | Keep/Move | Selection Signal |
|---------|-----------|------------------|
| ThreeStepHorizontal | ✅ KEEP | Simple default, 3 steps |
| VideoWalkthrough | ✅ KEEP | Needs demo video |
| AccordionSteps | ✅ KEEP | Technical product, detailed |
| VerticalTimeline | ✅ KEEP | Duration/time-focused |
| ZigzagImageSteps | ✅ KEEP | Needs step images, alternating |
| AnimatedProcessLine | ✅ KEEP | Playful/bold vibe |
| VisualStoryline | ✅ KEEP (from BeforeAfter) | Timeline + images variety |
| IconCircleSteps | ❌ REMOVE | Too similar to ThreeStepHorizontal |
| CardFlipSteps | ❌ REMOVE | Complex, AnimatedProcessLine is cleaner |
| Timeline (from Features) | ❌ REMOVE | Covered by VerticalTimeline |

**Selection Logic:**
```
hasDemoVideo → VideoWalkthrough
isTechnicalProduct → AccordionSteps
hasStepImages → RANDOM(ZigzagImageSteps, VisualStoryline)
vibe=playful/bold → AnimatedProcessLine
else → RANDOM(ThreeStepHorizontal, VerticalTimeline)
```

### Testimonials (8 UIBlocks → 5 + 3 incoming)

| UIBlock | Status | Selection Signal |
|---------|--------|------------------|
| QuoteGrid | ✅ KEEP | Default, simplest |
| RatingCards | ✅ KEEP | Ratings/platform focus (G2, Capterra) |
| VideoTestimonials | ✅ KEEP | Needs video testimonials |
| AvatarCarousel | ✅ KEEP | Interactive, friendly vibe |
| PullQuoteStack | ✅ KEEP | Emotional vibe, masonry |
| BeforeAfterQuote | ✅ INCOMING from BeforeAfter | Transformation testimonials |
| SegmentedTestimonials | ✅ INCOMING from UseCases | Segment-based testimonials |
| StripWithReviews | ✅ INCOMING from SocialProof | Reviews strip |
| QuoteBackedAnswers | ✅ INCOMING from Objection | Expert quote answers |
| InteractiveTestimonialMap | ❌ REMOVE | Too niche, needs global data |

**Selection Logic:**
```
hasVideoTestimonials → VideoTestimonials
hasRatings → RatingCards
hasPersonas → SegmentedTestimonials
hasTransformationStory → BeforeAfterQuote
else → RANDOM(QuoteGrid, AvatarCarousel, PullQuoteStack)
```

### Problem (7 UIBlocks → 3)

| UIBlock | Status | Selection Signal |
|---------|--------|------------------|
| CollapsedCards | ✅ KEEP | Default, expandable problem list |
| SideBySideSplit | ✅ KEEP | Path comparison framing |
| PainMeterChart | ❌ REMOVE | Needs numeric pain data |
| ProblemChecklist | ❌ REMOVE | Too interactive |
| EmotionalQuotes | → Testimonials | Problem quotes = testimonials |
| PersonaPanels | ✅ KEEP | Persona problems |
| BeforeImageAfterText | → BeforeAfter | Visual comparison |

**Selection Logic:**
```
hasPersonas → PersonaPanels
else → RANDOM(CollapsedCards, SideBySideSplit)
```

### SocialProof (8 UIBlocks → 6)

| UIBlock | Status | Selection Signal |
|---------|--------|------------------|
| LogoWall | ✅ KEEP | Default, customer logos |
| UserCountBar | ✅ KEEP | Growth/momentum story |
| StackedStats | ✅ KEEP | Simple metrics |
| MediaMentions | ✅ KEEP | Press coverage |
| SocialProofStrip | ✅ KEEP | Compact option |
| IndustryBadgeLine | ✅ KEEP | Certifications (B2B) |
| StripWithReviews | → Testimonials | Reviews = testimonials |
| MapHeatSpots | ❌ REMOVE | Too niche, needs global data |

**Selection Logic:**
```
hasCertifications → IndustryBadgeLine
hasPressMentions → MediaMentions
hasCustomerLogos → RANDOM(LogoWall, SocialProofStrip)
else → RANDOM(StackedStats, UserCountBar)
```

### Results (9 UIBlocks → 7)

| UIBlock | Status | Selection Signal |
|---------|--------|------------------|
| StatBlocks | ✅ KEEP | Default, core metrics |
| OutcomeIcons | ✅ KEEP | Professional outcomes |
| TimelineResults | ✅ KEEP | Temporal progression |
| ResultsGallery | ✅ KEEP | Visual/creative tools |
| StackedWinsList | ✅ KEEP | List format variety |
| EmojiOutcomeGrid | ✅ KEEP | Playful vibe |
| BeforeAfterStats | → BeforeAfter | Transformation = BeforeAfter |
| QuoteWithMetric | → Testimonials | Quotes = testimonials |
| PersonaResultPanels | ✅ KEEP | Persona results |
...
else → RANDOM(StatBlocks, OutcomeIcons, StackedWinsList, EmojiOutcomeGrid)
```

### UniqueMechanism (9 UIBlocks → 7)

| UIBlock | Status | Selection Signal |
|---------|--------|------------------|
| SecretSauceReveal | ✅ KEEP | Default, marketing-friendly |
| StackedHighlights | ✅ KEEP | Vertical layout variety |
| TechnicalAdvantage | ✅ KEEP | Tech audience |
| AlgorithmExplainer | ✅ KEEP | AI/algorithm products |
| MethodologyBreakdown | ✅ KEEP | Framework products |
| PropertyComparisonMatrix | ✅ KEEP | Competitor comparison |
| ProcessFlowDiagram | ✅ KEEP | Behind-the-scenes process |
| SystemArchitecture | ❌ REMOVE | Overlaps with TechnicalAdvantage |
| InnovationTimeline | ❌ REMOVE | Too niche, history rarely needed |

**Selection Logic:**
```
hasCompetitorComparison → PropertyComparisonMatrix
hasProprietaryAlgorithm → AlgorithmExplainer
hasFramework → MethodologyBreakdown
isTechnicalProduct → TechnicalAdvantage
else → RANDOM(SecretSauceReveal, StackedHighlights, ProcessFlowDiagram)
```

### Pricing (8 UIBlocks → 5)

| UIBlock | Status | Selection Signal |
|---------|--------|------------------|
| TierCards | ✅ KEEP | Default, standard tiers |
| ToggleableMonthlyYearly | ✅ KEEP | Monthly/yearly toggle |
| SliderPricing | ✅ KEEP | Usage-based pricing |
| CallToQuotePlan | ✅ KEEP | Enterprise "contact us" |
| FeatureMatrix | ✅ KEEP | Complex feature comparison |
| CardWithTestimonial | ❌ REMOVE | Testimonials separate |
| MiniStackedCards | ❌ REMOVE | TierCards handles compact |
| SegmentBasedPricing | ❌ REMOVE | Too complex |

**Selection Logic:**
```
pricingModel=usage-based → SliderPricing
pricingModel=enterprise → CallToQuotePlan
hasMonthlyYearly → ToggleableMonthlyYearly
else → RANDOM(TierCards, FeatureMatrix)
```

### ObjectionHandle (8 UIBlocks → 3)

| UIBlock | Status | Selection Signal |
|---------|--------|------------------|
| VisualObjectionTiles | ✅ KEEP | Default, visual grid |
| BoldGuaranteePanel | ✅ KEEP | Guarantee/risk-reversal |
| MythVsRealityGrid | ✅ KEEP | Myth-busting |
| ObjectionAccordion | → FAQ | Accordion format → FAQ handles |
| ObjectionCarousel | ❌ REMOVE | Accordion better for Q&A |
| ProblemToReframeBlocks | ❌ REMOVE | Overlaps MythVsReality |
| QuoteBackedAnswers | → Testimonials | Expert quotes |
| SkepticToBelieverSteps | ❌ REMOVE | Too complex |

**Selection Logic:**
```
hasGuarantee → BoldGuaranteePanel
else → RANDOM(VisualObjectionTiles, MythVsRealityGrid)
```
...
| UIBlock | Status | Selection Signal |
|---------|--------|------------------|
| AccordionFAQ | ✅ KEEP | Default, expandable |
| TwoColumnFAQ | ✅ KEEP | Grid layout |
| InlineQnAList | ✅ KEEP | Simple list |
| SegmentedFAQTabs | ✅ KEEP | Categorized FAQs |
| ChatBubbleFAQ | ✅ KEEP | Conversational variety |
| IconWithAnswers | ❌ REMOVE | Redundant with InlineQnAList |
| QuoteStyleAnswers | ❌ REMOVE | Expert attribution rare |
| TestimonialFAQs | ❌ REMOVE | Testimonials separate |

**Selection Logic:**
```
hasFAQCategories → SegmentedFAQTabs
vibe=friendly → ChatBubbleFAQ
else → RANDOM(AccordionFAQ, TwoColumnFAQ, InlineQnAList)
```

### UseCases (8 UIBlocks → 4)

| UIBlock | Status | Selection Signal |
|---------|--------|------------------|
| UseCaseCarousel | ✅ KEEP | Default, generic use case cards |
| IndustryUseCaseGrid | ✅ KEEP | Industry-specific targeting |
| PersonaGrid | ✅ KEEP | Avatar grid, who uses it |
| RoleBasedScenarios | ✅ KEEP | Role + scenario rows |
| InteractiveUseCaseMap | ❌ REMOVE | Click-reveal too complex, not scannable |
| CustomerJourneyFlow | → HowItWorks | Marketing funnel, not use cases |
| WorkflowDiagrams | → HowItWorks | Process steps, not use cases |
| BeforeAfterWorkflow | → BeforeAfter | Before/after comparison |

**Selection Logic:**
```
hasDistinctIndustries → IndustryUseCaseGrid
hasPersonas → RANDOM(PersonaGrid, RoleBasedScenarios)
else → UseCaseCarousel
```

*Note: PersonaPanels, PersonaResultPanels, PersonaJourney stay in their original sections (Problem, Results, BeforeAfter) - they are contextualized by section purpose.*

---

### FounderNote (8 → 4)

| UIBlock | Status | Reasoning |
|---------|--------|-----------|
| LetterStyleBlock | ✅ KEEP | Classic personal letter, default |
| StoryBlockWithPullquote | ✅ KEEP | Long-form narrative |
| VideoNoteWithTranscript | ✅ KEEP | Video modality, accessibility |
| SideBySidePhotoStory | ✅ KEEP | Photo-driven story |
| FounderCardWithQuote | ❌ REMOVE | Too simple |
| MissionQuoteOverlay | ❌ REMOVE | Values, not story |
| FoundersBeliefStack | ❌ REMOVE | Beliefs, not story |
| TimelineToToday | ❌ REMOVE | Company history, no fit |

**Selection Logic:**
```
hasFounderVideo → VideoNoteWithTranscript
hasFounderPhotos → SideBySidePhotoStory
else → RANDOM(LetterStyleBlock, StoryBlockWithPullquote)
```
...
| UIBlock | Status | Reasoning |
|---------|--------|-----------|
| LeftCopyRightImage | ✅ KEEP | Default, most versatile |
| CenterStacked | ✅ KEEP | Simple, early-stage focus |
| ImageFirst | ✅ KEEP | Visual-first, unique layout |
| SplitScreen | ✅ KEEP | Premium/bold vibe |
| Minimalist | ✅ KEEP | No CTA, brand-focused |

**Selection Logic:**
```
vibe=premium/bold → SplitScreen
vibe=minimal/awareness → Minimalist
...
| UIBlock | Status | Reasoning |
|---------|--------|-----------|
| NavWithCTAHeader | ✅ KEEP | Default, standard + CTA |
| MinimalNavHeader | ✅ KEEP | Simple pages, fewer sections |
| CenteredLogoHeader | ✅ KEEP | Unique layout, design-focused |
| FullNavHeader | ❌ REMOVE | 7 nav items overkill for single-page |
...
| UIBlock | Status | Reasoning |
|---------|--------|-----------|
| CenteredHeadlineCTA | ✅ KEEP | Default simple |
| CTAWithBadgeRow | ✅ KEEP | Enterprise trust badges |
| SideBySideCTA | ✅ KEEP | Dual-path conversion |
| CountdownLimitedCTA | ✅ KEEP | Urgency mechanism |
| VisualCTAWithMockup | ✅ KEEP | Visual mockup |
| ValueStackCTA | ✅ KEEP | Value summary / closing argument |
| CTAWithFormField | ❌ REMOVE | Form is button config, not layout |
| TestimonialCTACombo | ❌ REMOVE | Testimonials have own section |

**Selection Logic:**
```
hasUrgency → CountdownLimitedCTA
...
### Close (8 → 0) - ELIMINATED

Section removed. MultistepCTAStack moved to HowItWorks. All others removed as duplicates or not useful.
...
| UIBlock | Status | Reasoning |
|---------|--------|-----------|
| ContactFooter | ✅ KEEP | Single flexible footer |
| SimpleFooter | ❌ REMOVE | User removes elements from ContactFooter |
| LinksAndSocialFooter | ❌ REMOVE | Covered by ContactFooter |
| MultiColumnFooter | ❌ REMOVE | Covered by ContactFooter |

**One footer with optional elements:**
- Logo, Copyright (always)
- Social links, Contact info, Newsletter (optional/removable)

---

### Persona Consistency Rule

**Critical:** If strategy defines personas (e.g., "Marketing Manager", "Sales Director", "Ops Lead"), those SAME personas must appear consistently across all persona-aware sections:

| Section | Persona-Aware UIBlocks | Content |
|---------|----------------------|---------|
| **UseCases** | PersonaGrid, RoleBasedScenarios | What each persona uses it for |
| **Problem** | PersonaPanels | Pain points per persona |
| **Results** | PersonaResultPanels | Outcomes per persona |
| **BeforeAfter** | PersonaJourney | Transformation story per persona |
| **Testimonials** | SegmentedTestimonials | Quotes from each persona type |

**Implementation:**
1. Strategy defines persona list once (part of "One Reader" decision)
2. All persona-aware UIBlocks pull from same list
3. Copy generation knows which persona it's writing for
4. Consistency creates cohesive narrative across page
...
| Results | 9 | 7 | -2 |
| Features | 8 | 6 | -2 |
| HowItWorks | 8 | 7 | -1 (+1 from Close) |
| SocialProof | 8 | 6 | -2 |
| FAQ | 8 | 5 | -3 |
| Testimonials | 8 | 9 | +1 |
| UniqueMechanism | 9 | 7 | -2 |
| Pricing | 8 | 5 | -3 |
| ObjectionHandle | 8 | 3 | -5 |
| UseCases | 8 | 4 | -4 |
| FounderNote | 8 | 4 | -4 |
| Hero | 5 | 5 | 0 |
| Header | 4 | 3 | -1 |
| CTA | 8 | 6 | -2 |
| Close | 8 | 0 | -8 (eliminated) |
| Footer | 4 | 1 | -3 |
| **Total** | **~124** | **~88** | **~29% reduction** |
...
#### Simulation 1: Embr (Debt Payoff App)

**Context:**
- Audience: Overwhelmed consumer, low-medium sophistication, emotional
- Landing goal: signup (free tier)
- Offer: Free basic, $6/month Pro
- Assets: Testimonials ✅, Results ✅, Logos ❌

**Objection Sequence:**
```
1. "Will it work for MY mess? I have 7 accounts."
   → Testimonials (people with messy debt like me)

2. "How does it actually work?"
   → HowItWorks (simple flow)

3. "Do I have to connect my bank?"
   → FAQ

4. "Is my data safe?"
   → FAQ (already added)

5. "30% faster sounds good. Prove it."
   → Results

6. "Show me real people."
   → Testimonials (already added)

7. "What does free get me?"
   → FAQ (already added)
...
**Result:** `Header → Hero → HowItWorks → Results → Testimonials → CTA → FAQ → Footer`

**8 sections.** AI naturally skipped UniqueMechanism (too technical) and Features (not asked for).

---

#### Simulation 2: Trophy (Developer API)

**Context:**
- Audience: Technical developer, high sophistication
- Landing goal: signup (free tier)
- Offer: Free up to 1k MAU, $29/month after
- Assets: Testimonials ✅, Logos ✅, Results ❌

**Objection Sequence:**
```
1. "I could probably build this myself."
   → UniqueMechanism (edge cases you haven't thought of)

2. "What edge cases exactly?"
   → UniqueMechanism (same section)

3. "Is the API actually clean? Show me."
   → HowItWorks (code snippets, SDK)

4. "Can I customize how it looks?"
   → Features

5. "Who else uses this?"
   → SocialProof (logos)

6. "Any developer testimonials?"
   → Testimonials

7. "What's pricing? Any lock-in?"
   → Pricing + FAQ

8. "Let me try the API."
   → CTA
```

**Result:** `Header → Hero → UniqueMechanism → HowItWorks → Features → SocialProof → Testimonials → Pricing → FAQ → CTA → Footer`

**11 sections.** Developer audience needed more detail — AI included it naturally.

---

#### Simulation 3: Toma (Automotive B2B)

**Context:**
- Audience: Vendor-fatigued dealership GM, high skepticism
- Landing goal: free-trial
- Offer: 30-day free trial, $9/seat/month
- Assets: Testimonials ✅, Logos ✅, Results ✅

**Objection Sequence:**
```
1. "Every vendor says 'never miss a lead.'"
   → SocialProof (dealer logos)

2. "How is this different from the last AI chatbot?"
   → UniqueMechanism (built for dealerships, you control it)

3. "What if the AI says something wrong?"
   → HowItWorks (control panel, message visibility)

4. "Can I see what it's saying?"
   → HowItWorks (already covered)

5. "Does it work with my CRM?"
   → Features

6. "40% more appointments? Prove it."
   → Results

7. "Show me dealers like me."
   → Testimonials
...
**Result:** `Header → Hero → SocialProof → UniqueMechanism → HowItWorks → Features → Results → Testimonials → Pricing → CTA → FAQ → Footer`

**12 sections.** Vendor-fatigued audience = more objections = more proof needed.

---

#### Simulation 4: Userology AI (User Research Tool)

**Context:**
- Audience: Time-starved PM, analytical
- Landing goal: free-trial
- Offer: 14-day free trial, $49/month
- Assets: Testimonials ✅, Logos ✅, Results ✅

**Objection Sequence:**
```
1. "Can AI really capture nuance like a human researcher?"
   → UniqueMechanism (synthesis, not just transcription)

2. "How does it work? Do users talk to a bot?"
   → HowItWorks
...
5. "What do PMs like me say?"
   → Testimonials

6. "Is $49/month worth it?"
   → Pricing
...
**Result:** `Header → Hero → UniqueMechanism → HowItWorks → SocialProof → Results → Testimonials → Pricing → CTA → FAQ → Footer`

**11 sections.**

---

#### Simulation 5: Zone (Mac Focus Timer)

**Context:**
- Audience: Design-conscious Mac user, medium sophistication
- Landing goal: buy (one-time $4.99)
- Offer: $4.99 one-time, no subscription
- Assets: Testimonials ✅, Logos ❌, Results ❌

**Objection Sequence:**
```
1. "Looks pretty. But I already have a timer."
   → UniqueMechanism (dock integration, native Mac, not electron)

2. "What does it do?"
   → Features

3. "Is it good? Reviews?"
   → Testimonials

4. "$4.99, one-time?"
   → Hero/CTA (simple price)

5. "OK, buy it."
   → CTA
```

**Result:** `Header → Hero → UniqueMechanism → Features → Testimonials → CTA → Footer`

**7 sections.** Simple product, low price, few objections.
...
| Pattern | What AI Did |
|---------|-------------|
| Fewer objections | Fewer sections (Zone: 7 vs Toma: 12) |
| More skepticism | More proof sections (Toma needed logos + results + testimonials) |
| Technical audience | UniqueMechanism + detailed HowItWorks (Trophy, Userology) |
| Emotional audience | Skip technical explanations (Embr skipped UniqueMechanism) |
| Simple/cheap product | Shorter page (Zone: $4.99 = 7 sections) |
| High-ticket B2B | Longer page with heavy proof (Toma, Userology) |
| Strong offer | Shortcircuits objections (waitlist + great deal = fewer sections) |

---

### Key Principles for Option B

1. **AI simulates real objections** — not hypothetical buckets
2. **Each section earns its place** — must answer a real objection
3. **Deduplication is natural** — same section answers multiple objections = one section
4. **Order follows mental sequence** — objection order = section order
5. **Context drives everything** — audience, offer, assets inform AI judgment
6. **No rules to maintain** — AI adapts to any product type
7. **Simpler is better** — fewer abstraction layers = fewer failure points
...
| Card count | Respect UIBlock limits | Schema defines min/max, AI stays in range |
...
1. **High-traffic UIBlocks first** — Hero, Features, Testimonials, CTA
2. **Common combinations** — blocks that appear on most pages
3. **Edge cases last** — rarely used blocks

Both reviews required before MVP lock.
