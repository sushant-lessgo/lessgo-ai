...
> Track doc for the multi-page capability, driven by the naayom customer. PO verdict: **Phase 1 approved**; corrections folded below.
...
- **Phase 4 — Port TechPremium archetype blocks (designer package).** Staged: **4a chrome+behaviors** ✅ DONE · 4b Home blocks · 4c Gallery + Contact. Recreate each section from `design_handoff_naayom/` as dual-render block pairs against `Naayom - Block Specs.html`. Reuse tokens; behaviors via the shared **`naayom.v1.js`** asset; lead form → existing forms system; icons → lucide; preserve `data-surface` + `<em>`.
  - **Phase 4a — STATUS: ✅ DONE + GATE PASSED (deployed to prod 2026-06-21).** Shared `naayom.v1.js` behaviors asset (dropdown nav, mobile menu, shared lightbox, gallery filter, live readout tick) built by `buildAssets.js` + injected by `htmlGenerator` for `templateId==='techpremium'`; subsumes Phase-3's per-block product-gallery inline script (removed). Reusable Readout card (`blocks/Readout`). Header upgraded (Products dropdown + mobile panel + Platform-login + Book-a-demo). Footer upgraded (brand blurb + click-to-action contact + socials + legal) + floating **WhatsApp FAB**. Registration smoke-test guard. Fix-ups deployed: **`--wa` token** added to `tokens.ts` (FAB + "Ask on WhatsApp" green were invisible); **publish self-heals stale `redirect:` KV** (clears dangling subdomain→custom-domain 301s on republish when no live custom domain). **Verified on published URL:** WhatsApp FAB visible+green, mobile burger panel toggles with nav links. (Gotcha: blob-proxy `s-maxage=3600` → hard-refresh/incognito after republish.)
