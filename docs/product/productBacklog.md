...
34. **Inline email capture in hero (single-field)** — when the conversion action only needs an email, render the email field + button DIRECTLY in the hero (type + submit in one place), not a bare CTA button; multi-field stays button→scroll-to-form-section (per form-CTA presentation pattern). This is the REAL problem the deleted `convertCTAToForm` feature stabbed at — do it properly. From editor-defect-fixes (2026-07-17)
