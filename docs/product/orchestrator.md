...
**STRATEGY (founder ruling):** no customer waiting → **build everything, QA once properly, push once.** No incremental pushes. Local `main` is **~150 commits ahead of origin** (origin/main = current live prod, untouched). The one pre-push gate = **`docs/product/deploy-qa-checklist.md`** (living master QA doc; a **preview deploy is MANDATORY** — Stripe/publish/KV/Resend/domains/middleware don't run locally). **All merged worktrees are PRESERVED** until the one deploy is green, then bulk-cleanup (do NOT clean per-merge anymore — batch strategy).

**Building now (1 active session):**
- **`work-onboarding-ingestion` (E2)** — at **P3**. Plan approved rev3; P1+P2 merged-in-branch (parity gate PASSED). **See "E2/E3 seam" below — I corrected it to rebase onto E3's merged seam.** Next founder touch = its **P5 Kundius real-photo gate**. D7b: E2 merges **ZERO prod-reachable behavior** (pilot-only; enablement = atelier-cutover).
...
**⚠️ E2/E3 SEAM INCIDENT + RULE (2026-07-17):** E3 merged FIRST and widened the founder-signed journey seam (`engines/journey/engines/types.ts`) with a **`questions()` data method** (STEP 03 = data-driven, agnostic `StepQuestions.tsx` renders). E2 at P3 is adding **`showWork.loadStep?` component-injection** (STEP 02 ingestion is too rich for data descriptors). **Two extension mechanisms now coexist on the seam — this is an accepted ruling** (data-driven for Q&A steps; component-injection for rich interactive steps). E2 was told to **merge main first, rebase onto E3's `questions()`, add `loadStep?` on top, do NOT revert/duplicate E3's work.** **RULE for E4 + any future engine step: hand it the seam contract AT LAUNCH — reuse `loadStep?`/`questions()`, never a third widening.** Lesson saved: `memory/feedback_shared_contract_coordination` (coordinate shared founder-signed contracts at LAUNCH, not via async mailbox — sessions go idea→merge in <1h).

**Specced + QUEUED (blocked — do NOT launch yet):**
- `atelier-skeleton-cutover` (full) — **launch after E2 merges** (deletes the `atelier2` id E2 pilots on). Re-points `atelier`→work-skeleton + deletes old `templates/atelier/`. Founder confirmed NO precious atelier draft. **This one cutover unblocks BOTH E2 enablement AND the toolbar's real action sets + Form/Menu hosts.**
- `work-onboarding-plan` (E4, **unwritten**) — after E2 **and** E3. Hand it the seam contract at launch.
- Toolbar action-sets + Form/Menu hosts (**unwritten**) — after the cutover.

**Owed before the push → all in `docs/product/deploy-qa-checklist.md`:** preview-deploy QA of prod-only surfaces (Stripe/publish/domains/emails/middleware); `billing-correctness` live funded-gen smoke; editor-shell founder QA + 3 sign-offs; blog-composer GATE A; content-baseline Deploy B; toolbar 🔴 defects (dead sanitize / double-modal a11y / `convertCTAToForm` crash); E3 STEP01→03 walk; lead-reply draft-quality eyeball; final re-green + parity/generation specs. (Regen-verify now covered by toolbar-beta-followup's e2e.)
...
Summary: ✅ merged 15 · 🔨 building 1 (E2@P3) · ⏸️ paused 2 (blog-composer=GATE A, content-baseline=Deploy B) · 📋 specced/queued 1 (atelier-cutover) · ⬜ to spec/later = the rest.
...
| L2 work (E2) | Onboarding | `work-onboarding-ingestion.spec.md` | 🔨 **building @ P3** — pilots on `atelier2`; adding `showWork.loadStep?` **rebased onto E3's merged `questions()` seam** (2 mechanisms coexist). P5 = Kundius real-photo gate. Merges ZERO prod-reachable behavior (D7b) |
