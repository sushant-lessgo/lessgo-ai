...
| 8 | `docs/task/work-library-board.spec.md` | Work-beta (chosen) — §8a "Your work" project-scoped dashboard board to manage photos as a library (groups, add/hide/move/reorder/cover) with display-by-reference (pages update on republish). **Largely a clone:** testimonial board pattern + E2 onboarding grouping UI + t7 MediaAsset pipeline (parts 2+3 exist; this is the missing "part 1" board). Cut: copy-gen slot-machinery + page-promotion + generic products board → backlog #37; work-photos-only. Full tier (dashboard surface + dual-renderer gallery group-ref + republish). No new designer screen (reuses onboarding grouping board). |
| 7 | `docs/task/bilingual-editing.spec.md` | Work-beta blocker (Kundius NL/EN delivery) — editor-shell phase 4 removed the Languages control → no way to edit the non-default locale + an `activeLocale` regen-lock trap. **Re-mount not rebuild** (i18n machinery fully wired: localeContent/localeConfig through save/load/publish/render). Fix: re-mount toggle (top bar) + settings (modal) **gated to bilingual projects only** (honors the removal reason) + reset activeLocale→default on load. Independent/twin-field mode only; naayom→Hindi + full i18nPlan stay deferred. Standard tier. |
| 6 | `docs/task/facts-work-writeback.spec.md` | Work-beta blocker (bug) — in-editor work story-interview submit 400s (`brief.facts.work is required`); makes `facts.work` reach the regen route so a work customer can regenerate their About/story from the editor. **Narrow** (just the 400); wide editor↔facts sync → backlog #36. Standard tier. Needs repro-confirm (scout). |
