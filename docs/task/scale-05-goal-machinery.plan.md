...
Make the picked goal actually shape the page. Today the goal is a dead wire: the wizard stores a legacy enum, the Brief never gets the goal parameter, copy prompts get one CTA-verb line, forms are manual, and M2/M3/M4 mechanisms have no machinery. This plan wires: goal-slot param collection → `Brief.goal.param` (+ writeback so publish-time GOAL_REF resolves), per-intent copy guidance (label + new `cta_subtext` + emphasis), M1 form auto-seed at generation (incl. `subscribe-newsletter` as an email-capture form) **plus a shared template-agnostic lead-form block so the seeded form actually RENDERS + SUBMITS on published pages of every template**, M2 deterministic WhatsApp prefill, M3 store-badges and M4 follow-strip as further **shared blocks** on the same infrastructure, and an intent-first wizard goal step (likelyIntents + AI pre-select + "other").
...
2. **Shared template-agnostic blocks in a NEW home** `src/modules/generatedLanding/sharedBlocks/` (no such dir exists today), resolved via a shared-block shim in BOTH component registries *before* template dispatch. **Firewall-safe split:** TWO registry files mirroring the existing componentRegistry split — `sharedBlocks/registry.ts` (edit `.tsx` twins ONLY, imported only by `componentRegistry.ts`) and `sharedBlocks/registry.published.ts` (server-safe `.published.tsx` twins ONLY, imported only by `componentRegistry.published.ts`). No file ever holds both twins, so no `'use client'` code can reach the static-markup path. Blocks style via CSS vars + `data-surface` so they inherit template tokens. Alternative (per-template block promotion) rejected: N templates × 2 files each, forever. **The infrastructure + the FIRST shared block (M1 lead-form) land together in Phase 5** (pulled forward per design call #7); `storeBadges` (Phase 7) and `followStrip` (Phase 8) then only ADD blocks on the existing rails. New goal section types are injected deterministically at generation (no AI copy). **Human gate at END of Phase 5** (working architecture + working published form).
...
5. **Param capture keys on INTENT where the intent needs a special shape, mechanism-generic otherwise.** Concretely: `download-app` (mechanism M3) captures TWO store URLs into `param.links[]` (Play + App Store), not the generic single-URL M3 branch — Phase 7's badge injector reads `param.links` and host-sniffs both stores. All other M3 intents use the generic single `param.url`.
6. **`subscribe-newsletter` is an M1 form** (email capture), per orchestrator decision — routed through Phase 4's form auto-seed (newsletter template: email + optional name). It is NOT handled by Phase 8's follow-strip. Only `follow-social` uses the M4 follow-strip. **Intent-specific mechanism override at writeback:** the frozen vocab meta says M4 (`goalIntentMeta['subscribe-newsletter'].mechanisms = ['M4']` in `src/modules/goals/vocabulary.ts`); product decision is M1 form. Do NOT edit vocabulary.ts — override via an explicit intent branch at the writeback layer AND in param capture (same pattern as the download-app/rsvp intent branches), so capture ↔ writeback ↔ hero GOAL_REF resolution all agree on M1 (`#form-section` anchor to the seeded form, never the M4 social/external path).
7. **Published-form render gap → Option A (RESOLVED, user-approved).** Phase 4 seeded M1 form DATA correctly, but review found forms do NOT render/submit on PUBLISHED pages for core templates (meridian/hearth/lex): `FormPlacementRenderer` is edit-only, those templates ship no published form block, and the hero `#form-section` scroll anchor targets an id that is never emitted. This is a PRE-EXISTING bug — manual form-wiring is equally broken there; only vestria + techpremium ship real published form blocks (`<form data-lessgo-form>` → `form.v1.js` → `/api/forms/submit`; reference: `src/modules/templates/vestria/blocks/Contact/VestriaLeadForm.published.tsx`). Fix: build the lead-form as a SHARED template-agnostic block on the shared-block infrastructure (design call #2), pulled FORWARD to Phase 5. One block, both renderers, styled via `data-surface` + CSS vars, renders from `content.forms[formId]`, published twin emits the `data-lessgo-form` contract. Every template (incl. bespoke + future) benefits; permanently closes the "template has no published form" bug class. Store-badges (M3) and follow-strip (M4) then reuse the SAME infra instead of each building their own.
...
- phase 9 intent-first wizard goal step: done (review loops 2, ship) — HUMAN GATE (UX) pending user sign-off. NB: product GoalStep lacks service's default-select mount effect (latent trap if a skip path is added later)
...
5. Tests — must include an end-to-end shape assertion: wizard `goalParam` for download-app → `legacyGoalToBriefGoal` → `brief.goal.param.links` is exactly what Phase 7's injector will read; and subscribe-newsletter → `brief.goal.mechanism === 'M1'`.
...
**Render-gap resolution note (post-review, RESOLVED — option A):** as-built, this phase delivered the seed DATA (forms + buttonConfig + wiring) but the rendered/published form for core templates does NOT exist — `FormPlacementRenderer` is edit-only and meridian/hearth/lex ship no published form block (pre-existing bug; see design call #7). The rendered + submitting form on BOTH renderers is delivered by **Phase 5's shared lead-form block**, which injects a real `leadForm-<uuid>` section carrying the `#form-section` anchor. Phase 4's CTA-button `#form-section` wiring is SUPERSEDED/COMPLEMENTED by that rendered section — no dangling anchor remains after Phase 5.

---

## Phase 5 — Shared-block infrastructure + M1 lead-form shared block  ⚠️ **HUMAN GATE (at END of phase)**

**Goal:** Build the shared template-agnostic block infrastructure (design call #2) and its FIRST block: the M1 lead-form. After this phase, an M1 wizard run on ANY template (incl. meridian/hearth/lex, where no published form block existed) produces a page where the seeded form RENDERS in the editor AND in exported published HTML, the hero GOAL_REF scroll anchor resolves to a real node, and a published-page submit hits `/api/forms/submit` → `FormSubmission` + lead email. Permanently closes the "template has no published form" bug class (design call #7).

**⚠️ Lowercase section-type keys:** both renderers' `extractSectionType` LOWERCASE the section type (verified: `componentRegistry.ts:28` and `componentRegistry.published.ts:23` both `.toLowerCase()`) — `leadForm-<uuid>` → `leadform`. `resolveSharedBlock`/`resolveSharedBlockPublished` must key on the lowercased type (`leadform`) — a cased key = silent non-resolution. Applies equally to `storebadges` (Phase 7) and `followstrip` (Phase 8).

**Files touched:**
- `src/modules/generatedLanding/sharedBlocks/registry.ts` — NEW: **edit twins ONLY** — `sharedBlockRegistry: Record<string, Component>` keyed by LOWERCASE section type (imports `.tsx` twins); `resolveSharedBlock(sectionType)`. Imported ONLY by `componentRegistry.ts`. Never imports a `.published.tsx` file.
- `src/modules/generatedLanding/sharedBlocks/registry.published.ts` — NEW: **published twins ONLY** — parallel map importing `.published.tsx` twins; `resolveSharedBlockPublished(sectionType)`. Imported ONLY by `componentRegistry.published.ts`. Never imports a `'use client'` file (firewall: keeps edit twins out of the static-markup path).
- `src/modules/generatedLanding/sharedBlocks/LeadForm/leadFormFields.tsx` — NEW plain module (**NO 'use client'**): renders an `MVPFormField[]` → labeled inputs/selects/textarea markup shared by both twins (model on vestria's `leadFormMarkup.ts` / `renderVestriaLeadField` + `contactFields.ts` defaults). Firewall-safe because it's a plain module.
- `src/modules/generatedLanding/sharedBlocks/LeadForm/LeadForm.tsx` — NEW ('use client' edit twin): reads `content.forms[form_id]` (form_id from section content elements), renders heading (editable via the standard inline-edit pattern) + the same field markup + submit button. Styled via CSS vars + `data-surface` only (no template imports).
- `src/modules/generatedLanding/sharedBlocks/LeadForm/LeadForm.published.tsx` — NEW (server-safe twin, identical layout/classes): emits `<form data-lessgo-form data-form-id={formId} data-page-id={publishedPageId} data-owner-id={pageOwnerId} data-success-message={form.successMessage}>` — **STUDY `src/modules/templates/vestria/blocks/Contact/VestriaLeadForm.published.tsx` and match its contract exactly**: data-attrs, submit-button/success-message fallbacks, and how `content`/`publishedPageId`/`pageOwnerId` arrive as props (the published renderer already passes them to every block — `LandingPagePublishedRenderer.tsx:146-147/191-192`). `form.v1.js` embeds automatically when `content.forms` is non-empty (Phase 4 guarantees that) and submits to `/api/forms/submit`. Both twins render a wrapper carrying **`id="form-section"`** so the existing scale-04 M1 GOAL_REF anchor + Phase 4's buttonConfig scrollTo resolve to this section (one leadForm per page — seed is idempotent).
...
- `src/modules/goals/seedGoalForm.ts` — EXTEND (chosen mechanism: extend Phase 4's module, NOT a companion injector — single seam, both GeneratingSteps already call it, idempotence logic already lives here): after seeding form data, also INJECT a `leadForm-<uuid>` section (into sections + sectionLayouts with layout `SharedLeadForm` + content elements `{ form_id, form_headline (manual_preferred, default from template heading) }`) positioned after hero (placement = open question #7). This rendered section is what the hero `#form-section` anchor scrolls to — Phase 4's `FormPlacementRenderer`-record write and CTA `#form-section` wiring are superseded/complemented by it (keep the buttonConfig wiring; drop/no-op the edit-only placement record if it conflicts — decide during impl, note in audit). Idempotent: no second injection when a `leadForm-` section already exists.
- `src/modules/sections/sectionList.ts` — add `leadForm` SectionMeta entry (labels/UIs don't show raw keys).
- `src/modules/audience/product/elementSchema.ts` + `src/modules/audience/service/elementSchema.ts` — minimal `leadForm` element entries (`form_id` manual, `form_headline` manual_preferred) so editor toolbars/element APIs tolerate the type.
- `src/modules/goals/seedGoalForm.test.ts` — EXTEND: seed injects `leadForm-<uuid>` section + layout + elements; anchor id contract; idempotence (no dupe section); M2–M5 still no-op.
- `src/modules/generatedLanding/sharedBlocks/__tests__/leadForm.parity.test.tsx` — NEW: edit vs published markup parity; resolution through BOTH split registries with a real `leadForm-<uuid>` id (lowercase-key regression); published `<form>` carries `data-lessgo-form`/`data-form-id`/`data-page-id`/`data-owner-id`/`data-success-message` + renders the seeded fields; `#form-section` anchor present in both renderers (test file may import both twins — tests aren't in the published path).

**Steps:**
0. **Grep readers that `switch`/branch on section type** (drag-drop, section-settings menus, regen menus, delete handlers, label switches, `getSurfaceForSection`) — enumerate every place a new section type must be tolerated or added; don't rely on `sectionList` + `elementSchema` entries alone. (This grep covers `leadform` now and pre-scouts for Phases 7–8.)
1. Split registries + componentRegistry shims — LOWERCASE keys per the note above (verify against both `extractSectionType` implementations); verify `getSurfaceForSection` isn't called for shared types, or defaults safely — shared blocks self-set `data-surface="neutral"` and style via CSS vars only.
2. Plain field-renderer helper module (study Vestria's core/markup/fields trio first).
3. LeadForm twin pair (identical layout; published data-attr contract per Vestria; `id="form-section"` anchor in both).
4. Extend `seedGoalForm` with section injection + resolve the Phase-4 placement-record/anchor supersession (no dangling anchor).
5. `sectionList` + both `elementSchema` entries.
6. Tests.

**Verification:** `npx tsc --noEmit` · `npm run test:run -- seedGoalForm leadForm componentRegistry dispatch formPlacement` (template-dispatch regression suite must stay green) · `npm run build` (published CSS must pick up block classes) · manual: consultant book-call wizard on meridian AND hearth → editor shows rendered form, hero CTA scrolls to it; publish → exported HTML contains `<form data-lessgo-form …>` inside `#form-section`; test submit on the LIVE published page → `FormSubmission` row + lead email (RESEND env).

**⚠️ Gate (END of phase):** present the working shared-block architecture (split registries + shims) AND a published core-template page where the seeded M1 form RENDERS and a test submit lands a `FormSubmission` + lead email. First shared block + delicate firewall + closes the M1 acceptance — user signs off before store-badges/follow-strip ride the same rails.
...
**Goal:** `storeBadges` section type: Play Store + App Store badge row (Kathaworld class), rendered identically by both renderers in ANY template via the **existing shared-block infrastructure from Phase 5** (split registries + componentRegistry shims — nothing infra-level is built here); injected at generation when intent = `download-app` and store links exist in `param.links` (captured by Phase 1's intent-specific branch); badges link out (M3), hero primary already resolves external via scale-04. Registry keys are LOWERCASE (`storebadges`) per the Phase 5 note.
...
- `src/modules/generatedLanding/sharedBlocks/registry.ts` — register `storebadges` edit twin (EXISTING file from Phase 5; lowercase key).
- `src/modules/generatedLanding/sharedBlocks/registry.published.ts` — register `storebadges` published twin (same rule).
...
**Steps:**
0. Re-run the section-type-switch grep (Phase 5 step 0) for `storeBadges` — tolerate/add the type everywhere enumerated.
1. Badge SVG art module.
2. Block twin pair (identical layout; links from section content elements; `data-surface` self-set, CSS vars only).
3. Register both twins in the EXISTING split registries (lowercase `storebadges` key — verify resolution with a real `storeBadges-<uuid>` id).
4. Injector + GeneratingStep wiring + sectionList/elementSchema entries.
5. Tests (parity test should exercise resolution through the registries, catching any case mismatch).
...
**Goal:** `followStrip` section: social-icon row rendered from injected socialProfiles content (writers class), on the **Phase 5 shared-block infrastructure**; hero primary = social Destination (scale-04 M4 already resolves); strip anchors are beacon-tagged conversions. **`follow-social` ONLY — `subscribe-newsletter` is M1 and handled by Phases 4–5 (resolved).**
...
- `src/modules/generatedLanding/sharedBlocks/registry.ts` — register `followStrip` edit twin (lowercase key `followstrip` — see Phase 5 lowercase-key note).
...
- `src/app/onboarding/service/[token]/components/steps/GeneratingStep.tsx` — pass socialProfiles/param into injector call (arg only; injector wired in phase 7).
...
**Steps:** 0. Re-run the section-type-switch grep (Phase 5 step 0) for `followStrip`. 1. Icons module. 2. Block twins. 3. Injector extension + register in BOTH registry files (lowercased `followstrip` key, per Phase 5 note — else silent non-resolution). 4. Schema/list entries. 5. Tests.
...
## Phase 9 — Intent-first wizard goal step (unhide + extend)  ⚠️ **HUMAN GATE (after: UX sign-off)**
...
## Phase 10 — Acceptance fixtures + parity QA  ⚠️ **HUMAN GATE (final acceptance before merge)**
...
- `src/modules/goals/__tests__/acceptance.scale05.test.ts` — NEW fixture-driven suite:
  ...
  - consultant/book-call → `seedGoalForm` output: form in `content.forms`, `leadForm-<uuid>` section injected; **published markup (via the shared LeadForm published twin) contains `<form data-lessgo-form data-form-id …>` with the seeded fields, inside the `#form-section` anchor, submit target `/api/forms/submit`** — the form RENDERS + SUBMITS on a published page, not just an anchor; CTA wired; hero GOAL_REF resolves the anchor.
  - subscribe-newsletter → writeback stamps `mechanism='M1'` (override, not vocab M4); email-capture form seeded AND `leadForm-<uuid>` section injected; **published markup contains the rendered `data-lessgo-form` email-capture form** (renders + submits, same assertions as book-call); hero GOAL_REF resolves `#form-section`; NO followStrip injected.
...
**Verification:** `npx tsc --noEmit` · `npm run test:run` (FULL suite) · `npm run build` · manual `/manual-test` subset: editor↔published parity for hero subtext, leadForm, storeBadges, followStrip on meridian + hearth; **live published-page form submit → FormSubmission + lead email**; wa.me link opens with prefilled text on a phone. **Gate: user signs off acceptance + merges (no-PR workflow: build/tsc/tests green locally before push).**
...
- Shared-block registry keys are LOWERCASE (`leadform`, `storebadges`, `followstrip`) — both `extractSectionType` implementations lowercase the type; a cased key = silent non-resolution (Phase 5 note).
- No `prisma db push`; no schema.prisma change is expected at all in this feature (Brief/content are JSON columns).
- `npm run build` (not bare `next build`) whenever published-block classes/CSS or assets change (phases 2, 5, 7, 8, 10).
...
1. ~~Shared-block home + split registries OK?~~ — **RESOLVED: option A approved (design call #7); infra + lead-form land in phase 5; working-architecture gate at END of phase 5.**
...
7. leadForm section placement: after hero (matches other injectors, short scroll) vs conventional bottom/CTA position — confirm at phase-5 gate.
