...
## Merge reconcile (main → feature/template-factory) — 2026-07-12
- Held merge-to-main per founder (queued behind the 3-feature bundle: tracking-pixels/app-subdomain/social-posts/email-sequences/cold-outreach). Bundle landed on main @138e35b1.
- Committed approved docs housekeeping first (9ea47896: 37 completed-task archives + queue renumber).
- `git merge main` → merge commit fbcf4927. CODE auto-merged CLEAN (htmlGenerator.ts / renderPublishedExport.ts / publish/route.ts — tracking-pixel head-injection + my phase-3 knobs threading are ADDITIVE, both survived: verified 3+6 / 6+5 refs, zero markers). Only conflict = docs/product/productBacklog.md (item 15 social-posts) — resolved to main's side (information-preserving, list stays consistent).
- Re-green ALL gates post-merge: `tsc --noEmit` clean; `test:run` 2424 passed / 1 failed (known i18n flake — passes 15/15 in isolation, pre-existing) / 11 skipped (bundle's new tests now in suite, count 2285→2436); `npm run build` green; `playwright test parity render` 6 passed (hearth ≤0.249%, parityBreak 6.409% caught).
- STATUS: feature/template-factory reconciled with main + green. NOT merged to main (founder holds the merge). Awaiting founder QA gate pack sign-off (phase 8/9/11) before merge-to-main.
