...
**D9 — Seam widening: `steps.showWork` gains an optional lazy step component (engine-agnostic).** ← human gate (E1's seam contract was founder-signed)
Widen `JourneyEngineSeam.steps.showWork` to `{title,body,icon, loadStep?: () => Promise<{default: ComponentType<JourneyStepProps>}>}`. The agnostic frame (`StepShowWork.tsx`) renders the engine component via `React.lazy` when `loadStep` exists, else the stub — other engines keep the stub. Firewall holds: loader invoked at RENDER time on STEP 02 (post-confirm), never at seam load; `journeyAgnostic.test.ts` stays green.
