...
- phase 5 D5 demand board + serve-gate demand wiring: done (commit <pending5>, impl-review 1 loop → ship; place/quick-yes never→brief.copyEngine (unit+e2e), demand-lead contract byte-identical, no migration, both paths (D4-place + serve-gate manual)→D5, go-back→D4. FOUNDER GATE OWED: dev place→D5→real DemandLead row + founder email. All green, e2e 5 passed)
