...
**One honest sentence (CORRECTED at the phase-2 gate — the pre-phase-2 version overclaimed on three counts):** the shell/anatomy half of the spec's Goal ships for **SIX** element families — Text, Button/CTA, Image, Section, Footer, Social — **not seven: Form does NOT dispatch** (over-determined dead, see the Form row); the curated-action-set half ships **less than half** — Footer, Menu and Form deliver no new capability, Section loses Background, Social loses Orientation, Image loses Link, Button "Style" is **absent** (not a relabel), Ask AI is gate-optional. **What genuinely lands:** the t2 shell + `ToolbarButton` unification (four hand-rolled button impls → one primitive), `LinkTargetPopover` killed across 14 sites, Link on **Text + Button/CTA only**, Social manage-items, form-field reorder inside FormBuilder, and one real crash fix (`showFormBuilder` threw `TypeError` ⇒ FormBuilder was 100% unreachable — see phase-2 note; **phase 4's premise depends on this fix**).
