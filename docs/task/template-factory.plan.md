...
- phase 8 hearth knobs pilot + looks data: done (commit 105506f1, review loops 2 [loop1=race false-negative, not a real fix]; ship — default byte-identical + :root untouched, both renderers consume knobs, looks rule bites live, tsc/2265 tests/parity ≤0.249%/build all green). **HUMAN GATE OPEN: founder no-visual-change sign-off on a real hearth draft before phase 9.** Runtime wiring (registry/LandingPageRenderer/publish caller) MOVED INTO phase 9 scope.
- phase 9 looks in picker: done (commit e0d493d4, review loops 1; runtime wiring RE-PROVEN end-to-end by reviewer — published HTML gets data-knob+CSS for non-default look, none for default/null; zero copy-regen; back-compat intact; onboarding tiles=variant+palette only [knobs=editor popover, judged acceptable — looks differ by palette/variant]; tsc/2266 tests/parity/build green). **HUMAN GATE OPEN: founder picker-UX sign-off before phase 10.** Cleanup nit: knob data-attrs camelCase (data-knob-buttonShape) — React warning, works via HTML case-insensitivity.
- phase 10 generation spread: done (commit cf4dfaa5, review loops 1; determinism+distribution genuine [FNV-1a→mulberry32, no Math.random, 10 seeds→≥7 distinct, same seed→identical], explicit-pick guard real, dormant styleAutoAssign flag = genuine 1-line toggle to working path, firewall+back-compat confirmed; tsc/2285 tests). Spread DORMANT until wizard slot sets styleAutoAssign — recommend founder gate at activation (changes new meridian+hearth starting look/palette).
- phase 11 skill rewrite + manual-test subsection: done (commit PENDING, review loops 1, ship; new-template SKILL 640→201 lines, zero code-derivable facts [deleted ids grep 0 hits], 14 pointers verified live, decision gate matches scalePlan §7, 8 landmines retained, manual-test §5b covers phase-2 human-half). **HUMAN GATE: founder reads skill rewrite before it governs builds.**
