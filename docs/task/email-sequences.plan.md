...
- phase 7 final verification + merge: build+tsc+tests GREEN (npm run build ✅, tsc 0, 1810 tests); AT MERGE HUMAN GATE. COUPLING: branch based on feature/social-posts → merging to main brings 8 social-posts commits + 5 email commits. social-posts is ungated + has NO kill-switch (standing rule: don't deploy ungated). email has kill-switch (default enabled). Pilot copy NOT human-validated. Merge decision: HOLD (user, 2026-07-10) — do NOT merge yet. Resolve social-posts gating/kill-switch (+ pilot validation + pricing-v2 gating) before merging both together. Branch left green + resumable. To resume: worktree .claude/worktrees/feature-email-sequences (isolated node_modules, own .env), branch feature/email-sequences (5 email commits on feature/social-posts). Dev DB already has EmailSequence table (harmless until merge); prod migration rides deploy.
