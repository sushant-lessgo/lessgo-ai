...
- B9 logo renders badly in editor (want it good everywhere): diagnosed → implementing. Root cause: shared `Logo.tsx` still uses boxed square `/logo.svg`; qa-0718 only fixed dashboard sidebar (transparent `/lessgo-logo.png`). Fix = update shared `Logo.tsx` to transparent wordmark + aspect-correct sizing (fixes editor+onboarding+marketing). NOT dual-renderer. Tier standard.
- B10 can't find unpublish (CLOUD_OFF icon leak makes it look broken): diagnosed. Root cause: `cloud_off` ligature absent from self-hosted Material Symbols subset → literal text → row width blowout truncates label. Fix = `ProjectCardMenu.tsx:277` name="cloud_off"→"visibility_off" (verified present); regression = subset-manifest test. Tier hotfix (skip review).
