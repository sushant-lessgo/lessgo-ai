...
- phase 7 publish flow (t17) + publish.spec selector repair: **built** (commit 970c2056, impl-review loops 1 → `ship`; gates: tsc 0, 3337 tests, lint 0, e2e 22 passed/4 skipped/0 failed with publish.spec EXECUTION proven) — **AWAITING FOUNDER MANDATORY PUBLISHED-PARITY GATE**
  - **Parity = argument-by-construction, NOT a byte diff.** Nobody has held two exported HTMLs side by side (needs `git stash`, which the implementer can't run). Chain verified link-by-link by impl-review: payload built in `handlePublish` = byte-identical vs HEAD · renderer/`staticExport`/`kvRoutes`/`/api/publish`/`/p/[slug]`/blob-proxy/`og`/middleware/templates/every `.published.tsx` = zero diff · **the only DOM-derived input (`htmlContent`) is THROWN AWAY** (`route.ts:212,246` write `htmlContent:''`; `generateStaticHTML` has NO caller in this path — blog only) · `.app-chrome` is class-scoped, never applied on `/p/*`, not compiled into `published.css`. Residual risk very low but it is reasoning, not measurement.
  - **PHASE 8 CARRIES (added):** `app-chrome.css:11`'s contract blanket-forbids `/preview/*` but phase 7 legitimately attaches there (canvas-SIBLING wrappers, `display:contents`, phase-3/4 precedent) → **amend the comment to "any wrapper containing the canvas"** or a future reader reads phase 7 as a violation · `publish.spec.ts:62-65` comment overclaims ("enabled whether or not the nudge is showing") but never asserts the nudge rendered → assert `publish-review-nudge` visible, or soften the comment · 3 undisclosed copy drops: **"Last updated: `<date>`" gone** (`existingPublished.publishedAt` now a DEAD prop field, typed `SlugModal.tsx:35`, never read), "Page Title (for SEO & social sharing)"→"Page title", "{n}/60 chars (recommended for SEO)"→"{n} / 60" — founder call on the recency signal · two modal-overlay treatments coexist (Radix `bg-app-ink/60` vs phase 7's hand-rolled `rgba(20,20,40,.45)`) — keeping the hand-rolled shell was CORRECT (Radix Dialog would add focus-trap + scroll-lock = behavior change) but it's a visual-consistency item · `publish-publishing-card` (20s) relies on local dev publish being slow — load-bearing + undocumented, could flake on a fast publish.
  - **`CLAUDE.md` IS STALE** (confirmed, out of scope): it says publish runs `generateStaticHTML()` + uploads to Blob. That has no caller in the landing publish path (only `src/lib/blog/publishBlogPost.ts:138`). Worth a doc fix in some later track.
- phase 8 PageSwitcher + final sweep + gates: **done** (commit 6b997528, impl-review loops 1 → `fix first` → fixed; gates: tsc 0, 3337 tests, lint 0, **build GREEN**, published.css isolation 0 bytes)
  - impl-review blocking: **the new `setOpen(false)` killed double-click-rename** — click 1 closed the menu, the 150ms `animate-out` unmounted the row, click 2 landed on nothing. `renamePage` had exactly ONE caller and no reliable trigger left; NOT a founder-authorized removal (multi-page = techpremium = naayom). **Defended by a correct-but-irrelevant audit claim** ("row + delete are SIBLINGS so double-click-rename survives" — true, and beside the point). Caught via the asymmetry: delete works (sibling + `stopPropagation`), rename didn't. Fixed with a pencil sibling; dead gesture removed, not left half-working.
  - **Third grey-out near-miss caught**: "Browse all" exists TWICE — `ThemePopover`'s → `StyleBrowserModal` (palette browser, WIRED, protected); Service/Vestria's → t14 template picker, no handler (correctly greyed). Verified by reading the modal's imports, not the label.
  - **`smartphone` renders as literal TEXT (144px)** — the subset build **silently DROPPED it** despite it being in `icons.txt` at build time (`6e7af964`). Manifest is wrong in BOTH directions (175 listed vs 247 shipped ligatures). Substituted `phone` + code note. Added `error` to `icons.txt` (`toast.tsx` renders it; missing from font AND manifest → a regeneration would NOT have fixed it; live trap for the dashboard track). **Regeneration must VERIFY output against the font's ligature table — the documented command is what dropped it.** Blocked here: no Python/fontTools on this machine (confirmed).

## MERGE READY — main merged in + re-greened (2026-07-16)

Merge commit `fcd40ff9` (main `b3bf9345`, 27 commits: auth-redesign + all 6 dashboard-workspace-ia phases + handoff docs).
- **Conflict**: `playwright.config.ts` `authed` testMatch — BOTH tracks had added specs. Resolved as a **union**. Notable: the dashboard track independently hit and documented the same allowlist trap ("an unregistered spec silently matches no project and gives false confidence") — the config's shape is the hazard; both warnings merged into one comment.
- **Re-green post-merge:** tsc **0** · **3402 tests passed** (+65 from main's dashboard suite) · lint **0** · build **GREEN** · `published.css` isolation **0 bytes** of app-chrome/app-coming/Onest/Material Symbols/app-ink/app-primary, byte-identical at 32031.
- **e2e post-merge:** `editor-dirty-guard` **4/4** ✓ · `publish` **3/3** ✓ (repaired t17 selectors) · `ui-isolation` **2/2** ✓ ("no app-chrome fonts/classes on the block surface") · `render` **3/3** ✓. **NOT re-run:** `parity.spec` (4 templates × ~3min) + `generation.spec` + main's dashboard specs — parity was green pre-merge, our diff is chrome-only, and canvas isolation is independently confirmed 4 ways (import graph, founder's published-HTML byte diff, `published.css` 0 bytes, `ui-isolation` green).
- ⚠️ **STALE DEV SERVERS COST 3 FALSE-RED e2e RUNS.** Implementers left `npm run dev` on `:3003`/`:3007`; Playwright reused a corpse serving HTTP 500 → `auth.setup` failed + all parity specs timed out at 3.0m, looking exactly like "the merge broke parity". `playwright.config.ts`'s own comment warns of it. **A red result lies as convincingly as a green one.** Always `netstat`-check the port before an e2e run in a worktree; kill only PIDs whose command line contains THIS worktree's path.
- Setup debts that came due at merge: `docs/Design/` was copied in as UNTRACKED at setup (main has since committed it → blocked the merge checkout; removed the copies); stale `.next/types` from before main re-homed the blog routes → `rm -rf .next`.
