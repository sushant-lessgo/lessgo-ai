...
- phase 11G HUMAN GATE B: CLEARED — founder-approved 2026-07-14 (reactivity + perf sign-off).
- phase 12 lint rule flip (+ HUMAN GATE C confirm): done (commit 93423d06) — GATE C founder-approved 2026-07-14 (pre-flight: zero bare code hits, only the 3 predicted non-code matches, zero exemptions). `.eslintrc.json` `no-restricted-syntax` added; negative test confirmed lint ERRORS on a scratch bare call; lint/tsc/test:run all green.
- phase 13 docs close-out: done (this commit) — editorPlan phase-4 row → DONE + Q2/Q3 + Decision-3 (ops-undo descoped to universe/i18n) + 6-never-mounted-modals-KEPT ruling; CLAUDE.md + 3 READMEs (edit/[token], hooks, stores) repointed to selector-first `useEditStore`; baseline close-out note.
- phase 13G HUMAN GATE D (final merge to main): pending (orchestrator-handled)
...
> **Re-scope (2026-07-12, post-merge, orchestrator):** actual bare `useEditStore()` = **73 text matches = 70 real call sites + 3 non-code** (2 in `useEditStore.ts`: a JSDoc example + the TS overload *signature* `export function useEditStore(): EditStore;` — a declaration, NOT a CallExpression, so the D3 AST lint rule ignores it; 1 in `LeadForm.tsx` an explanatory comment — its code uses a selector). The 70 real sites map EXACTLY onto batches B1–B6 below; only per-file counts rose vs the scout estimate — **B1 `useOptimizedEditStore.ts` = 26** (not ~20), B2 = 11 (LandingPageRenderer 1 + EditablePageRenderer 2 + SelectionSystem 5 + ElementDetector 3), B3 = 4 (useEditor 1 + SectionCRUD 3), B4 = 6, B5 = 11, B6 = 12. No orphan real sites; the main merge added none. **Phase-10/12 acceptance:** the zero-bare grep will still surface those **3 permanent non-code matches** — disposition them as JSDoc/overload-decl/comment (not call expressions); the AST lint rule stays green.
...
1. **Primary per-batch reactivity net (DECIDED, every Step-B batch — hot AND cold):** the **renderProbe browser smoke on that batch's touched surfaces** (the real over-narrow-selector detector: type/select/act → assert live DOM update), on top of the static floor `npx tsc --noEmit` + `npm run test:run` + `npm run lint`. **Orchestrator refinement (2026-07-12):** the authed `npx playwright test edit-persistence` runs at the **two perf checkpoints only** (after phase 7 = hot paths done; after phase 10 = all done), NOT every batch — selector-ization never touches the commit/persist path, so per-batch persistence runs cost dev-server churn without proportional signal; the renderProbe (does the DOM update live?) is the reactivity-relevant check and runs every batch. Browser checks run **authed** — all three Clerk creds are confirmed present in the worktree `.env.local`; implementers run the probe on a FREE port (3020+), never killing the founder's :3000. No batch ships on tsc/test/lint alone. (Degraded-env note: if a future environment lacks creds the per-batch net degrades and the Gate-B founder `/manual-test` pass is the backstop.)
