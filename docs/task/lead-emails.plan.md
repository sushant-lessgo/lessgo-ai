...
- phase 2 visitor auto-reply send path: done (commit a41bcdf7, review loops 0 — standard tier). tsc clean, 86/86 tests green. Sanitizer mutation-checked: neutering `sanitizeNameToken` fails 7 tests, so the Decision-7 asserts are live, not inert.
  - **Escalation check CLEARED**: no field whitelist strips unknown form keys (`validation.ts:40` `finalContent: z.unknown().optional()`; `saveDraft/route.ts:194-199` shallow spread; `persistenceActions.ts:214-221` shape-guard only; `loadDraft/route.ts:118-120,150`). No persistence edits needed; tier stays standard.
  - **Pre-existing bug surfaced (own ticket needed)**: the submit route reads form config at `Project.content.forms[formId]`, but the editor exports forms at `Project.content.finalContent.forms` (`persistenceActions.ts:619`). Top-level only matches legacy pre-`finalContent` projects — so `formName` is stored as `'Unknown Form'` and ConvertKit integrations likely never fire for current sites. Phase 2 added a SEPARATE `autoReplyForm` lookup that checks both locations; `formConfig` was deliberately NOT widened (widening would change stored `formName` and start firing integrations that don't fire today — a behavior change outside this spec).
  - Logged deviations: `sanitizeNameToken` DELETES markup chars rather than entity-escaping (stronger than plan wording; `escapeHtml` still wraps the HTML body); local `isValidEmail` copy (the existing one is private inside `sendLeadNotification.ts`); typed-email-field pick does not fall back to `data.email` when the typed field's value is invalid; link stripping intentionally over-eager.
- phase 3 FormBuilder auto-reply settings UI: done (commit pending-at-write-time, see git log; review loops 0 — standard tier). tsc clean, lint clean, 103/103 tests green (FormBuilder 11 → 17). Anti-inert proof: stripping `autoReply` in `handleSave` fails 3 new tests; inverting ON-by-default fails 2.
  - Judgment calls: an untouched section saves `autoReply` as ABSENT (not `{enabled:true}`), matching the server's ON-by-default; plain checkbox (matches the two existing toggles in this modal) rather than a Switch; section sits above the "Start from a template" picker per the plan's literal placement, so loading a template still resets a new form's whole draft incl. auto-reply — pre-existing behaviour, left unchanged and flagged.
  - Reviewer watch-items: the `'use client'` → `@/lib/email/autoReplyTemplate` import is guarded only by comments (that module must stay zero-import); this UI is a no-op if phase 2's dual-location `finalContent.forms` lookup is reverted.
