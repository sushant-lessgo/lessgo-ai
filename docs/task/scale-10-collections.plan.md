...
- phase 2 scrape extraction → facts.collections: done (commit 2f48b817, review loops 1, ship) — thing/manufacturer→products, trust→services+case-studies, work→services+works; slugs code-derived; DROP on empty; understand-path yields empty arrays discarded by DROP.
- phase 3 serve gate + requiredCollections + demand granularity: done (commit aa88e42d, review loops 1, ship) — HUMAN GATE resolved: keep DORMANT (requiredCollections populated for NO businessType per founder 2026-07-10); mechanism + granular `collection:<key>` tag + ManualOnboard copy shipped; flip a type on later via one-line config when a block pair nears.
- phase 4 structure gate 7b collection node: done (commit 7efdbd8e, review loops 2, ship) — collapsible collection node (keys present ∪ requiredCollections), rename/remove/add name-only; buildBriefPatch emits COMPLETE facts snapshot (sibling-facts survive shallow-partial saveDraft merge; briefFacts seeded at hydrate, no concurrent facts writers); fix: entry <li> re-keyed idx (slug-in-key remounted input → focus loss).
- phase 5 generation→collections bridge (dormant, fixture-tested): done (commit 839d53d7, review loops 1, ship; HUMAN GATE passed — founder go 2026-07-10) — double gate (getCollectionDef(K) ∧ template declares K); dormant (no live template fires; fixture-tested); 2-layer clamp (iterate Brief entries + slug-intersect merge, verbatim record fields); naayom builders byte-identical (new generic builders); charge FLAT; resume via completedPageKeys. DEFER (rung-C): bridge trusts entry.slug rather than re-deriving slugify(name) inline — equivalent today; add belt-and-suspenders when live.
- phase 6 editor panel generalization: done (commit c0086120, review loops 1, ship) — ProductsModal keyed by collectionKey via getCollectionDef; products pixel-identical (deriveLabel reproduces inner+outer fallback; singularNoun keeps copy byte-identical); panel opened per collectionKeysInPages (products excluded from manage loop, keeps its button+event). DEFER (rung-C): (a) module-scoped _panelCollectionKey not reset by lessgo:manage-products event → stale key once non-products panels live (1-line reset in GlobalModals); (b) pageActions.addCollectionItem hardcodes buildProductDetailSlice + 'New product' default → generalize per def.itemSectionType when block pairs land.
- phase 7 conformance test (declared collection capability ⇒ block pair): done (commit pending-below, review loops 1, ship) — quantified assertion (declares family cap ⇒ capabilitySections cover catalog+item section types ∧ both resolve non-placeholder edit+published), vacuous today; dormancy lock + vestria regression lock (issue-1); 2 contained negative fixtures prove it bites via the real helper (reviewer empirically verified by injecting a cap into a live template). TEST-ONLY.
