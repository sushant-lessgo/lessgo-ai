...
- phase 9a design-system + Header + Hero + slider-markup: done (commit a9f430a3, review loops 2, verdict ship) — real Kontur tokens/palettes; knob values realigned (cardStyle=[hairline,flat], density=[comfortable,compact]); Header overlay+solid+drawer+page-head-mode; Hero slider DOM contract (data-atl-* hooks, no-JS first-slide) + marquee. FIXED density=compact editor↔published parity divergence (compact block redeclares final --pad-y/--sec-y directly, not via --space multiplier — --space resolves at declaring scope; published wrapper ≠ :root) + guard test. **CARRY→9b: palette ids — types/service.ts AtelierPalette still vermilion/indigo/olive; cobalt/moss values sit under indigo/olive + ochre absent → rename to vermilion/cobalt/moss/ochre + add 4th (touches service.ts, palettes.ts, paletteSelection.ts, imageKeywords.ts, PALETTES_BY_TEMPLATE). CARRY→phase 12/13: buttonShape zero-knob default = rounded (platform law) NOT Kontur square — surface at parity gate (per-project knob seed 'square'?).** Slider data-attribute contract for phase 10 is verbatim in audit.
- phase 9b content blocks (Work/Packages/About/Quote/Contact/Footer): done (commit b28d7bcb, review loops 1, verdict ship) — 6 blocks ported (mosaic+masonry gallery, 2/3/4 packages, bio-split+press About, dark quote grid, shared lead-form Contact, wordmark Footer+closer); palette ids renamed vermilion/cobalt/moss/ochre (+ochre added). **CARRY→phase 11: (a) unfilled NON-SCHEMA chrome keys (closer_headline/location/instagram/caption/more_text/index_heading/legal_text/press_items/studio_*/quotes) render their DESIGN PLACEHOLDER as live text — BLOCK_MOCKS must populate + phase-13 manual-fill must set them or placeholders reach the live Kundius page; (b) Work category-filter bar DROPPED (shared collection primitive has no per-item data-cat hook — needs a primitive/resolve change = orchestrator call); (c) registry.ts knobs mapping + index.ts:20 comment (phase-6 carry). CARRY→phase 12 parity eyeball: About collapses design's 3 surfaces (bio/press/studio) into 1 section; Contact single-col vs design 2-col; buttonShape default rounded≠Kontur square; Bricolage opsz axis.** — Bricolage woff2 (fontsource, /fonts/bricolage-grotesque/) + @font-face (wght 200-800) + preload case (editorial→Bricolage, compact→Fraunces) + SELF_HOSTED_FONTS whitelist; family-name 3-way match; `npm run build` exit 0
- phase 9 [HUMAN GATE] visual block port from approved designer HTML: pending
- phase 10 hero-slider behaviors asset: done (commit 8a42d501, review loops 1, verdict ship)
  > Phase 12 parity note: run `npm run test:parity` via a WARM dev server + `--project=public` (default globalSetup Clerk + cold build > 10min timeout). Dot-injection watchpoint resolved OPTION (b): hero mock = 1 static slide, both editor + slider.v1.js no-op dots for <2 slides → honest like-with-like compare. **Phase-12 Files-touched EXPANDED (orchestrator ruling): `src/modules/templates/atelier/blocks/editPrimitives.tsx`** — parity caught a REAL divergence: atelier Link primitive renders edit chrome (lg-atelier-link-edit wrapper + LinkTargetPopover) UNCONDITIONALLY (no mode gate) vs published plain `<a>` → atelier/header 4.99%>3%. Fix = mode-gate Link (+ make Img upload-overlay pixel-neutral) like Meridian gates on `mode==='edit'`. This is the parity form of phase-11's noted Img/Link open-risk. — atelierSliderBehaviors.js→slider.v1.js (1438b min); htmlGenerator usesAtelier gating (tested atelier-only); editor .tsx display:contents wrapper (parity-neutral, slide-1 baseline, focus-pause). **CARRY→phase 12 PARITY HARNESS: editor injects .lg-atelier-dot (steady state) but published STATIC snapshot has empty [data-atl-dots] (dots JS-injected on live page only) → harness must run slider.v1.js in the published frame OR use a <2-slide hero mock, else editor(dots) vs published-static(no dots) diverge for ≥2-slide mocks. Also: runtime slider behavior (autoplay/arrows/dots/JS-off) needs manual/e2e QA at phase 12.**
- phase 11 editor basics + atelier block mocks: done (commit d16f7584, review loops 1, verdict ship within scope) — 10 mocks (8 sections + packages 2/3/4), assertEditorBasics enrolled (non-vacuous +76 assertions), fixed AtelierEditable data-edit-primitive marker (real gap), registry knobs mapping + index.ts:20 comment (phase-6 carries). **CRITICAL FINDING → phase 11b (HARD blocker before 12/13):** chrome keys NOT in serviceElementSchema → extractLayoutContent DROPS them on read → NOT hand-editable (edits revert to hardcoded design placeholder) NOR populatable → design strings ("Let's make yours."/"@studioname") leak to live page. assertEditorBasics is marker-only, doesn't catch round-trip. Deferred carries: Work category-filter (needs shared-primitive data-cat hook); 7 Gate-0 punch-list = shared-editor → phase-12 manual QA; Img/Link/List mode-gating polish.

- phase 11b chrome-key schema-backing (HARD pre-12/13 blocker): done (commit 4f3eb591, review loops 1, verdict ship) — all atelier chrome elementKeys/collections added to Atelier* serviceElementSchema layouts (closer_*/location/instagram/legal_text/footer_links/badge_text/press_items/more_text/quotes/slides/etc.), all manual_preferred (inert to skeleton gen); round-trip persistence assertion added (closer_headline/legal_text survive extractLayoutContent + negative case); hearth/lex/generation untouched. Leak CLOSED. CARRY→phase 12 manual QA: collection round-trip (slides/press_items/quotes/footer_links) only marker-count-tested not round-trip-asserted; marquee_items decorative (no elementKey, DEFAULT fallback) — inert. — add atelier chrome elementKeys to `src/modules/audience/service/elementSchema.ts` Atelier* layouts (Contact: location/instagram; Footer: closer_headline/closer_cta/contact_location/index_heading/elsewhere_heading/legal_text/footer_links; About: badge_text; Work: more_text/caption; Quote: headline; etc. — grep the 6 cores for every hardcoded-placeholder E.Txt/E.Img elementKey NOT already in schema). Then re-populate those in blockMocks + add a ROUND-TRIP persistence assertion to editor-basics (marker-only green missed this). Atelier-scoped (Atelier* layoutNames — doesn't touch hearth/lex).
- phase 12 parity enrollment + [HUMAN GATE] parity QA sign-off: AUTOMATED DONE (commit pending-below, review loops 1, verdict ship) — atelier in TEMPLATES; screenshot parity ALL sections <3% (header 1.72/hero 0.74/work 0.14/pkgs 2-3-4 <0.25/about 0.13/quote 0.00/contact 0.06/footer 0.87); both negative controls fire (~6.5%); content parity green; FIXED real divergence (Link/Img/List edit primitives mode-gated to byte-match published). **⏸️ HUMAN GATE PENDING = founder parity sign-off** (slider runtime autoplay+arrows+dots+no-JS on published; bilingual EN/NL toggle+geo; knob/palette live-switch both renderers; Dutch typography D5; eyeball About-3→1-surface / Contact single-col; DECISIONS: buttonShape default rounded vs Kontur-square seed, Bricolage opsz-axis font swap; collection round-trip slides/press/quotes/footer_links; 7 vestria Gate-0 shared-editor findings re-check).
- phase 12b parity fidelity (opsz font + square default seed): done (commit 293ace65, review loops 1, verdict ship)
- phase 12 [HUMAN GATE] parity QA sign-off: ✅ SIGNED OFF by founder 2026-07-12 (automated <3% all sections + orchestrator visual QA pass + live palette-switch demoed). buttonShape=square + opsz font approved & landed (12b).
- MAIN MERGED IN + re-green (2026-07-12, commit a84939fc): main 9f03c996 fully contained. Clean merge — 1 trivial conflict (htmlGenerator.ts destructuring: kept removeBranding+usesAtelier); useWizardStore.ts auto-merged w/ hydrate semantics intact. Post-merge reconcile: repointed 2 atelier code files + renderParity mock off deleted useEditStoreLegacy → canonical @/hooks/useEditStore (imports only). Re-green: tsc0 / test:run 2775 passed/15 skipped / build0 / prisma generate ✓. Founder ruled PROCEED on all 4 prod behavior changes. Reported GO to orchestrator mailbox → station feature→main merge fires there (I don't push). Backup of a divergent untracked editor-phase-4 spec in scratchpad/premerge-backup (flagged to editor session).
- phase 13 [HUMAN GATE] Kundius EN/NL manual-fill + publish: DEFERRED (founder: "Kundius later"). Content/ops only, ZERO platform code. Needs: Kundius EN base copy + NL overlay + real images (delivery/assets/ has candidates), then onboard photographer Brief→serve→5-page skeleton→manual-fill→publish; custom domain attach when she's ready. Live smoke to fold in: slider autoplay/arrows/dots + no-JS fallback on the published page; EN/NL toggle + geo. **CODE BUILD COMPLETE through 12b; template registered + serveable + parity-green. Merge to main = separate human gate (user pushes).**
