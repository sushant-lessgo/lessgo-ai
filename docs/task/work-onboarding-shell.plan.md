...
- phase 1 rail data model + entry→work seed + brief plumbing: done (commit 715e4c63, review loops 1, verdict ship) — AWAITING HUMAN GATE (rail shape sign-off)
...
| WHAT YOU SELL | `facts.work.groups[]` (names, prices as chips) | exists; seeded from `facts.entry.offerings`, each **`{name, kind:'category', price:{mode:'on-request'}}`** — `kind` REQUIRED by `WorkGroupSchema` (decision 4 / landmine 6). **Groups carry `photos`/`items` through `WorkGroupInput` + `normalizeWorkGroup`** (founder ruling, phase-1 amendment): rail group edits must never wipe E2-ingested photos |
...
| LANGUAGES | `facts.work.languages[]` (NOT `brief.locales`) | exists in the MODEL; **NOT rendered in E1** — no source (nothing infers it; E1 asks nothing). See P3 founder ruling |
| carried (model, not rendered E1) | `establishment`, `praise`, `contactMethod`, **`location`/`reach`**, **`languages`** | generation branches on them; E2 (ingestion → location + site) / E3 (answers) fill + render. `location` has no E1 source: `EntryFacts` carries no location field (`classify.ts:97-113`) |
...
2. `UnderstoodRail.tsx` per handoff: 312px fixed aside (steps 02–06), mono header "WHAT WE UNDERSTOOD" + "Tap anything to correct it", field blocks — **E1 renders EXACTLY FOUR: NAME / WHAT YOU DO / WHAT YOU SELL (chips) / PRICE POSITION** — unknown = `opacity-50` + `bg-app-stripes` skeleton bar (E1: only PRICE POSITION can legitimately show it, until STEP 03 collects a price), trailing `edit` affordance → inline input (thin, no dialog), footer "Something wrong?" free-text box → `appendUserNote`. Chips = badge pill/status variants; toasts from `@/components/ui/toast` (landmine 12).
   - **🚦 FOUNDER RULING (phase-1 gate) — WHERE + LANGUAGES are NOT rendered in E1.** The handoff mock shows six fields; E1 has **no source** for two of them. `EntryFacts` (`classify.ts:97-113`) carries `rawInput`/`summary`/`businessName`/`offerings`/`audiences`/`categories`/`deliveryModel` but **no location** — "I photograph weddings in Amsterdam" keeps the location only inside `rawInput`, unextracted. And nothing infers language (the track rule is "auto from location + their site" — E1 has neither, and E1 asks for neither). A rail headed "WHAT WE UNDERSTOOD" must not present a hardcoded default as a belief we formed (same honesty rule as price-position-shows-unknown-not-`middle`); a permanent skeleton is worse — it implies "filling soon" and never fills.
   - **The rail MODEL keeps `location`/`reach`/`languages`** (built in P1, unrendered) — E2 (ingestion, gives location + site → language inference) and E3 (answers) render them when they have a source. Do NOT delete them from `rail.ts`; do NOT render them in E1.
   - **Known accepted consequence:** generation still silently defaults primary language to `en` (`slimStrategy.ts` — `languages[0] ?? 'en'`), so a Dutch pilot (Kundius) gets English copy with nothing surfacing the assumption. Accepted for E1; **added to the P7 founder-QA checklist** and it is E2/E3's job to close. Do not "fix" it here.
