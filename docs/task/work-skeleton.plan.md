...
- phase 5 published pipeline (work.v1.js · htmlGenerator · editor behaviors) — pilot gate: done (review loops 1 + 3 parity fixes, ship). work.v1.js (slider+fixed-header; lightbox deferred to ph6). PILOT GATE: passed on automated parity proof — all 7 atelier2 bands <3% (hero 0.015%, deterministic ×3), negative controls bite. Founder VISUAL eyeball vs designer Atelier HTML = PENDING review (flagged, non-blocking). Parity harness exposed+fixed 3 in-flow editor-chrome bugs in phase-2 editPrimitives.tsx (Link/Img/List → absolute overlays). CARRY-FWD (editor-UX papercut, non-block): hover-gated affordance buttons (.wk-list-add/.wk-list-x) capture clicks while opacity:0 — add pointer-events:none-until-hover in ph6.
- phase 6 layout library completion (header ×5 · hero ×3+slot · gallery ×3 · proof ×3 · sticky): done (review loops 1, ship). 17 atelier2 parity bands <3% (max 1.813%, deterministic). Header internal-dispatch (.toBe(default) ×4), hero ×3 distinct, gallery masonry/strip, proof logos/results (distinct copyShape), lightbox added, pointer-events fix. +2 benign out-of-list edits (coreParity count 6→13; e2e .first() on neg-control locator only — main test pairs by index, harness not weakened). Header hairline structural fix. 
- **[AC-L123 GAP — fix before phase 8]** static-export generator htmlGenerator.ts does NOT thread themeValues.styleTokens into LandingPagePublishedRenderer → hand-written styleTokens (radius/background) render in editor but NOT static-export published page. Edit renderer threads it (ph2); published RENDERER accepts it; only the generateStaticHTML generator + publish-route caller miss it. Inert today (no panel) but violates AC L123 "renders in BOTH renderers". FIXED (phase 6b, review ship): styleTokens threaded generator→renderer + 2 callers (publish route persists into publishedThemeValues; verify-dns regen). Byte-neutral (null→''). 3 AC-L123 tests bite. CARRY-FWD nits: (a) verify-dns still drops knobs (pre-existing, not regressed) — follow-up; (b) renderPublishedExport fan-out + publish-route sourcing only tsc-verified not runtime-tested.
- phase 7 section coverage + zero-markup/bounds conformance — freeze gate: done (review loops 1, ship). 4 blocks (Packages/About/Faq/Results), skinPurity+bounds tests bite (AC L121/L122, old-atelier excluded), renderParity.work, 21 bands <3%. DEVIATION ACCEPTED: bespoke NOT flipped (it also gates customer serve fit/shortlist → flipping exposes dev-only atelier2 to real customers pre-cutover); engine-core conformance enforced via explicit describe instead; REAL flip folded into phase 9 cutover (served id=atelier, non-bespoke naturally). Mailbox note left for founder. FREEZE GATE: passed on consolidated lint table (audit) — founder review PENDING. +2 mechanical test edits (coreParity 13→17, dispatch rows).
- phase 8 kundius end-to-end + parity QA — sign-off gate: done (review loops 1, ship). kundiusPages.test: 5 real Kundius pages (home/work/prices/about/contact) via REAL published path (generateStaticHTML) + edit path, symmetric XOR + real per-section markers (bites placeholder leak/divergence). NL/EN twin non-preclusion proven. No bug found (single-source = structural parity). 3269 tests green. SIGN-OFF GATE: automated proofs green; founder browser sign-off + real-content eyeball = PENDING (manual-QA checklist in audit). Nits: proof/metrics mock illustrative (concierge content-load), checklist item 3 could name Prisma Studio.
- phase 9 cutover to `atelier` id + docs close-out — gated: DEFERRED per orchestrator ruling (unattended run) — re-points LIVE Kundius prod rendering; needs explicit founder go at the sign-off gate. Includes the real bespoke flip. Post-merge follow-up.
