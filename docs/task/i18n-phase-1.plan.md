...
- phase 6 publish route + KV locale paths: done (commit 5a70b0bc, review loops 1) — **CODE done; FOUNDER LIVE GATE PENDING.** localeConfig+overlay read from DB Project.content; per-locale extraRoutes flow through existing atomicPublish/KV (no middleware edits); collision guard rejects subpage slug == declared non-default locale. Single-locale byte-identical. **Non-blocking:** base=payload / overlay=DB consistent via preview save-then-export (best-effort save; if it fails, overlay 1 autosave stale → older NL / EN fallback, republish fixes). Live-gate checklist in audit.
- phase 7 bilingual queryable + honesty test: done (commit 40eed2d7, review loops 1). Brief `locales?` in `brief.schema.ts` (not types/brief.ts — Brief is zod-inferred); derivation (locales>1 → bilingual) + `PLATFORM_CAPABILITIES=['bilingual']` satisfaction in `src/modules/templates/fit.ts`; retired/bespoke short-circuit runs first (retired templates stay non-fitting). Honesty test is a real lie-detector (RED if resolver unexported / switcher unregistered / head-tags dropped). serveGate unchanged.
- phase 8 generic en/nl pilot + docs: done (commit 7df5a92f, docs + final verification). i18nPlan.md marked Phase 1 BUILT + decisions + limitations; Lumen supersession noted. FINAL: tsc clean, test:run 2091 passed/3 skipped, `npm run build` green (switcher.v1.js emitted). Manual acceptance sweep = FOUNDER gate (pending). Atelier E2E lands in atelier build.
