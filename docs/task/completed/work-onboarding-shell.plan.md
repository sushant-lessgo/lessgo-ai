...
- phase 5 STEP 05 building (seam-driven generation): **done** (commit dc2558ef, review loops 1, verdict ship; e2e 7 passed; all 3 traps closed — `finalContent` chain real + proven, `onDraftCorrected` wired, kill-switch moved to the leaf). **2 PRE-EXISTING bugs found + review-verified, NEITHER blocks P6/P7 — see "Pre-existing bugs" below; founder rules at the merge gate.**

## Pre-existing bugs found in P5 (review-verified; NOT caused by this feature; NEITHER blocks the E1 gate)

**Bug A — work fan-out exceeds the shared AI rate limit.** atelier = 5 pages (`pageArchetypes.ts`, all `defaultIncluded`) ⇒ `work.llm.ts:127` 1 strategy + `:190-207` 5 copy = **6 sequential calls in seconds**. Both routes are wrapped by `withAIRateLimit` (`strategy/route.ts:205`, `generate-copy/route.ts:319`) and `defaultKeyGenerator` keys on `user:{userId}` with **NO route discrimination** ⇒ one shared bucket. FREE=5/min ⇒ the 6th deterministically 429s. **Reproduced live in review's e2e** (`"requests": 6, "limit": 5`); the 61s retry then completed. ⚠️ **Broader than FREE:** PRO=10 ⇒ only 4 calls of headroom — one regen in the same minute breaks PRO too. ⚠️ `rateLimitStore` is an **in-memory Map, per-lambda-instance on Vercel** ⇒ prod behavior is intermittent, not cleanly deterministic (harder to diagnose). **Does NOT block the E1 pilot gate** — per-page persistence + `completedPageKeys` resume + "Try again" recover it, and STEP 05 surfaces it honestly; it degrades run #1. **Owner: separate work on `rateLimit.ts`** (exempt the fan-out, or count a fan-out as ONE operation), before pilot polish. NOT this feature.

**Bug B — `finalize`'s marker delete never reaches Postgres.** `finalizeMultiPageGeneration` does `delete fc.generationProgress` (`multiPageAssembly.ts:268`) **client-side only**; `/api/saveDraft` merges `{...existingContent.finalContent, ...finalContent}` (`route.ts:194-199`) — a deleted key is merely ABSENT from the payload, so the shallow spread **preserves the stored value**. Pre-existing, platform-wide (affects `thing` too). ⚠️ **Severity corrected DOWN by review — the audit overstated it.** "The editor treats a finished site as mid-generation" is **NOT supported**: no editor code reads `generationProgress` (only `isResumableGeneration`, the work/thing skip-loops, `resumeStep.ts`, and `editDelta/capture.ts:98` where it's in a skip-list). Real consequence = a brief STEP-05 flash on reload that re-drives **chargelessly** (`work.llm.ts:265` returns `runFanOut` directly, skipping strategy; every page in `completedPageKeys` ⇒ 0 AI calls ⇒ re-finalize ⇒ 06) + permanent junk in stored `finalContent`. **Cosmetic + wasteful, not a handoff break; does not endanger P7's QA gate.** **Owner: low-priority follow-up on `/api/saveDraft` (tombstone support)** — belongs to whoever owns saveDraft, not this feature.
- phase 6 STEP 06 reveal + editor handoff (agnostic): **done** (commit 3fb85dfd, review loops 1, verdict ship; blocked mid-phase by global XFO `DENY` → founder ruled option B + wrote the header split; reveal e2e 7 passed + PROVEN, build exit 0, default preview byte-identical). **P7 inherits 3 notes — see P7 step 6.**
- phase 7 gates sweep + founder QA: **done** (commit 5c890ef1; XFO `:token*`→`:token+` = genuinely exclusive, proven both directions; ALL gates green — tsc, 3484 unit, lint, FRESH build, full e2e 45 passed, ui-isolation 2/2, tailwindConfigFreeze, fixture NOT regenerated, purity+drift guards). **AWAITING HUMAN GATE (founder QA) → then the merge gate.**
...
6. **Reveal isolation via iframe + `chrome=0` preview** — ⟳⟳ **FOUNDER-RULED 2026-07-16 (mailbox), option B — after the mechanism was CHALLENGED and re-decided. Read this before touching STEP 06.**
   - **The challenge:** the iframe was blocked by `X-Frame-Options: DENY` on `source: '/(.*)'` (`next.config.js:9-36`) — global, blocks framing EVEN SAME-ORIGIN, so the reveal renders blank. Runtime header ⇒ `npm run build` cannot catch it. Any same-origin route is blocked identically, so changing the framed route does NOT dodge it.
   - **And the repo has a NON-iframe precedent for app-chrome bleed:** the **route-group escape** — `src/app/(blog-preview)/dashboard/blog/[slug]/[postId]/preview/page.tsx` lives OUTSIDE `src/app/dashboard/` (route groups are URL-invisible) precisely so `dashboard/layout.tsx`'s `.app-chrome` is never an ancestor ("DO NOT move it back"); same in `BlogPostEditor.tsx:182`; and `/edit/[token]` renders the site INLINE with no iframe. **The house pattern is "escape the ancestor", not "iframe".** (This plan reached for the industry-standard builder iframe — Webflow/Framer/Squarespace all iframe their canvas — and 3 plan-review passes didn't catch the local precedent. The founder did.)
   - **RULING: iframe + `SAMEORIGIN` anyway — and it is the LONG-TERM-RIGHT call, not a shortcut.** Rationale (founder track context): the merged editor preview is getting a **mobile-view toggle** (`toolbarPlan.md` — "Mobile-view toggle + clean read mode"). An honest mobile preview needs a **TRUE VIEWPORT**, which only an iframe delivers — the inline route-escape squishes a window-width layout and cannot fire the site's real breakpoints. An iframe is therefore needed eventually regardless; build it once, now. `SAMEORIGIN` = own-origin frames own content, cross-origin clickjacking stays blocked — standard builder-tool posture.
   - **Refinement 1 (do if cheap, must not block):** scope the header **narrowly** — `SAMEORIGIN` on the framed preview route(s) only; keep the global `DENY` everywhere else (defense in depth). Do NOT flip `/(.*)` wholesale.
   - **Refinement 2:** **`/preview?chrome=0` is the INTERIM target.** Founder direction: generate + reveal + preview all **consolidate onto the edit route** — preview becomes a mode toggle in the editor, first-time generation becomes the editor's first-load state, and the standalone `/preview`/`/generate` surfaces RETIRE. That is future **editor-track** work, NOT E1's; this phase-6 iframe is the correct interim and meets E1's 01→06 gate now. **Do not hardwire "/preview is permanent" assumptions** — the iframe target later moves to the editor preview surface.
   - Mechanism as built (unchanged): `/preview/[token]` gains `chrome=0` → renders ONLY the site (no action bar/Publish/modals). **Primary and ONLY read: `useSearchParams` in a small child component under `<Suspense fallback={null}>`** (precedent `src/app/dashboard/billing/page.tsx:155`); the page is one `"use client"` component with NO Suspense today, so the wrapper is needed regardless — do NOT use a `window.location.search`-in-an-effect fallback (it first-paints the action bar INSIDE the reveal iframe, then removes it — a flash the founder will see). Local `npm run build` in the phase that edits it. `.app-chrome` is never an ancestor of the revealed site. STEP 06 is fully agnostic. E2E must assert the Publish control is **absent from the iframe DOM**, not merely hidden.
...
- ⟳⟳ **`next.config.js` (edit — FOUNDER-AUTHORIZED 2026-07-16, added mid-phase): relax `X-Frame-Options` DENY → `SAMEORIGIN` for the framed preview route ONLY, keeping the global `DENY` catch-all intact (refinement 1). This is a SECURITY-HEADER change — narrow it, verify the global default still applies to every other route, and prove the reveal actually frames (`curl -I` the route + the e2e in-frame assertions, which are written but were UNPROVEN while blocked).**
...
5. Fixes arising ONLY in files already listed in P1–P6 (which now includes `next.config.js`, added mid-P6).
6. ⟳ **P6 inheritance (review notes — do 6a, judge 6b, correct 6c):**
   - **6a. The `next.config.js` comment states a FALSE invariant — fix it (this is the instruction a future maintainer acts on).** It claims the two XFO sources "MUST stay mutually exclusive." They are NOT: `/preview/:token*` is **zero**-or-more so it matches **bare `/preview`**, and `/((?!preview/).*)`'s lookahead only rejects `preview/` — so bare `/preview` matches BOTH. The config is nonetheless functionally correct for a DIFFERENT reason: **Next dedupes `headers()` by key with LAST-WINS, so the real invariant is ENTRY ORDER (the DENY block must stay last)** — not exclusivity. Verified from the compiled `.next/routes-manifest.json`. Low actual risk (the overlap lands on a 404 — there is no `src/app/preview/page.tsx` — and the reveal target `/preview/{token}` matches only the SAMEORIGIN rule, so a reorder couldn't re-block it). **Fix EITHER way, don't leave the lie:** preferred = `/preview/:token+` (one-or-more) which makes them genuinely exclusive (verified: `+` ⇒ `/preview`=false, `/preview/abc`=true) AND makes the comment true; or keep `*` and rewrite the comment to name **ordering** as the invariant. This is a **security-header file** — narrow the change, keep the global `DENY` intact for every route but the framed preview, and re-prove with `curl -sI` (exactly ONE xfo line per route; preview ⇒ SAMEORIGIN, `/`+`/dashboard`+`/edit/{t}`+bare `/preview` ⇒ DENY).
   - **6b. Desktop reveal is 768px** (`JourneyShell`'s shared `max-w-3xl`), narrower than the handoff. NOT fixed in P6 because widening moves EVERY step — **needs a founder call at this gate.** Add it to the QA checklist.
   - **6c. Correct the audit's build-artifact note.** The missing `.next/routes-manifest.json` was NOT the custom `build:published-css`/`build:assets` prebuild steps: review reproduced it — `npm run build` produced the manifest fine, then **Playwright's `webServer: 'npm run dev'` overwrote `.next` with a dev build** (`BUILD_ID`/manifest gone, `.next/static/development` present). An ordinary dev/prod `.next` collision in one worktree. Benign, no deploy risk (CI builds clean). Fix the wording so nobody hunts a phantom build bug.
   - Also note (pre-existing, out of scope, do NOT fix): `ReferenceError: window is not defined` at `src/hooks/useEditStoreBootstrap.ts:238` on every dev SSR of `/preview` and `/edit` — dev-only (`NODE_ENV==='development'` guard), fires in `publish.spec.ts` too.
   - Minor, know it: `:token*` also covers `/preview/[token]/privacy` (a real route) ⇒ `SAMEORIGIN`. Benign (same-origin only, and it IS a preview surface). `:token+` keeps this behavior.
