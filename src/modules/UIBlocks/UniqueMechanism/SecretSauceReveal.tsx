...
export default function SecretSauceReveal(props: LayoutComponentProps) {
  ...
  return (
    <LayoutSection sectionId={sectionId} sectionType="SecretSauceReveal" backgroundType={props.backgroundType || 'primary'} sectionBackground={sectionBackground} mode={mode} className={props.className}>
      <div className="max-w-4xl mx-auto">
        <div className="bg-gradient-to-br from-purple-900 to-indigo-900 rounded-2xl p-12 text-white text-center relative overflow-hidden">
          ...
          <div className="relative z-10">
            ...
            <EditableAdaptiveHeadline mode={mode} value={blockContent.headline} onEdit={(value) => handleContentUpdate('headline', value)} level="h2" backgroundType="primary" colorTokens={{ ...colorTokens, textPrimary: 'text-white' }} textStyle={getTextStyle('h1')} className="text-white mb-6" sectionId={sectionId} elementKey="headline" sectionBackground="bg-purple-900" />
            <div className="bg-yellow-400 text-purple-900 px-6 py-3 rounded-full inline-block font-bold text-xl mb-6">
              <EditableAdaptiveText mode={mode} value={blockContent.secret_sauce} onEdit={(value) => handleContentUpdate('secret_sauce', value)} backgroundType="neutral" colorTokens={{ ...colorTokens, textPrimary: 'text-purple-900' }} variant="body" textStyle={getTextStyle('h3')} className="font-bold" sectionId={sectionId} elementKey="secret_sauce" sectionBackground="bg-yellow-400" />
            </div>
            <EditableAdaptiveText mode={mode} value={blockContent.explanation} onEdit={(value) => handleContentUpdate('explanation', value)} backgroundType="primary" colorTokens={{ ...colorTokens, textSecondary: 'text-purple-100' }} variant="body" textStyle={getTextStyle('body-lg')} className="text-purple-100 text-lg max-w-2xl mx-auto" sectionId={sectionId} elementKey="explanation" sectionBackground="bg-purple-900" />
          </div>
        </div>
      </div>
    </LayoutSection>
