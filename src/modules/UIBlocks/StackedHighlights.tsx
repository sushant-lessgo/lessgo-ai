            ...
            <div className="p-4 bg-blue-50 rounded-lg border border-blue-200">
              <div className="flex items-center text-blue-700 mb-3">
                ...
                <span className="text-sm font-medium">
                  StackedHighlights - Edit unique mechanism content or click individual elements above
                </span>
              </div>
              
              <div className="space-y-4 text-sm">
                <div>
                  <label className="block text-blue-700 font-medium mb-1">Highlight Titles (separated by |):</label>
                  <ModeWrapper
                    mode={mode}
                    sectionId={sectionId}
                    elementKey="highlight_titles"
                    onEdit={(value) => handleContentUpdate('highlight_titles', value)}
                  >
                    <div className="bg-white p-3 rounded border text-gray-800 text-xs leading-relaxed max-h-24 overflow-y-auto">
                      {blockContent.highlight_titles}
                    </div>
                  </ModeWrapper>
                </div>
                
                <div>
                  <label className="block text-blue-700 font-medium mb-1">Highlight Descriptions (separated by |):</label>
                  <ModeWrapper
                    mode={mode}
                    sectionId={sectionId}
                    elementKey="highlight_descriptions"
                    onEdit={(value) => handleContentUpdate('highlight_descriptions', value)}
                  >
                    <div className="bg-white p-3 rounded border text-gray-800 text-xs leading-relaxed max-h-40 overflow-y-auto">
                      {blockContent.highlight_descriptions}
                    </div>
                  </ModeWrapper>
                </div>
                
                <div className="text-xs text-blue-600 bg-blue-100 p-2 rounded">
                  💡 Tip: Icons are auto-selected based on highlight content (intelligent, dynamic, predictive, etc.). Connection lines appear between highlights on desktop for visual flow.
                </div>
              </div>
            </div>
          </div>
