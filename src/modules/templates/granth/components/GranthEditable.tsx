...
// src/modules/templates/granth/components/GranthEditable.tsx
// Contenteditable wrapper for Granth blocks. Given the block's content object + an
// element key, it reads/writes that key directly. Granth is Hindi-only — the Lumen
// language-aware twin-key routing is deliberately dropped. Wraps InlineTextEditorV2
// in `edit` (preserves inline HTML); static HTML otherwise. No colour tokens —
// caller controls styling via className/style.
...
interface GranthEditableProps {
  /** The block's resolved content object. */
  content: Record<string, any>;
  /** Element key to read/write (e.g. 'name', 'quote'). */
  elementKey: string;
...
export function GranthEditable({
  content,
  elementKey,
  mode,
  sectionId,
  ...
  if (mode !== 'edit') {
    // Static render. Preserve inline HTML (<em> gold separators etc.).
