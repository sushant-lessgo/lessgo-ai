...
export default function LogoStrip({ sectionId }: LogoStripProps) {
  ...
  const updateName = (id: string, value: string) => {
    handleCollectionUpdate('brands', brands.map((l) => (l.id === id ? { ...l, name: value } : l)));
  };

  const addLogo = () => {
    if (brands.length >= 8) return;
    handleCollectionUpdate('brands', [...brands, { id: `l${Date.now()}`, name: 'Client' }]);
  };

  const removeLogo = (id: string) => {
    if (brands.length <= 2) return;
    handleCollectionUpdate('brands', brands.filter((l) => l.id !== id));
  };
  ...
  return (
    <>
      ...
      <div className="sg-logos" data-section-id={sectionId}>
        {(blockContent.label || (mode === 'edit' && !isExcluded('label'))) && (
          <SurgeEditable
            as="span"
            mode={mode}
            sectionId={sectionId}
