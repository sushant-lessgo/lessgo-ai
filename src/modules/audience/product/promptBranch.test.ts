...
// - COPY_SAAS_BASELINE: rebaselined for proof-truth (named-company/hard-metric proof guard).
// - BOTH baselines: rebaselined for language-settings phase 4 — the output-language
//   directive is emitted UNCONDITIONALLY (plan ruling 2: gating it on non-English
//   would leave the reported "picked English, got Dutch" bug unfixed), so the
//   English prompts legitimately gained the `## OUTPUT LANGUAGE — English` block +
//   the unnumbered `**Write EVERY string in English.**` rule, and the strategy
//   prompt gained the one-line hedge. VERIFIED: those are the ONLY deltas vs the
//   previous baselines (line-diffed before re-freezing) — the zero-diff contract
//   covers storage and rendered output, NOT prompt text.
