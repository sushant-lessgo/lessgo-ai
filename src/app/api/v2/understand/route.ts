...
- oneLiner: one clear sentence describing what the business offers and who it's for (may lightly rephrase the input). Keep the specific niche terms from the source; do not substitute broader category words.
...
async function handleEntryUnderstand(
  ...
  // Credit PRE-GATE — check the balance BEFORE the AI call (one charging model:
  // check → generate → charge on success). Placed AFTER the demo/mock
  // short-circuit above, exactly where consumeCredits already applied, so demo
  // runs stay both uncharged and ungated. The failed attempt is still ledgered
  // here — it mirrors the success:false UsageEvent consumeCredits used to write
  // once the AI call had already run.
  const preCheck = await checkCredits(userId, CREDIT_COSTS.UNDERSTAND);
  if (!preCheck.allowed) {
    await logUsageEvent({
      userId,
      eventType: UsageEventType.UNDERSTAND,
      creditsUsed: 0,
      success: false,
  ...
  if (!creditResult.success) {
    // The charge failed AFTER the AI call → the output is DISCARDED (no free output).
    if (creditResult.error === 'charge_conflict') {
      ...
      logger.error(
        `[understand] Charge conflict for user ${userId} — nothing charged, output discarded`
      );
      return createSecureResponse(
        {
          success: false,
    ...
    return createSecureResponse(
      {
        ...
        creditsRequired: CREDIT_COSTS.UNDERSTAND,
...
async function understandHandler(req: NextRequest): Promise<Response> {
  ...
  try {
    ...
    // path. The legacy audienceType + per-template manufacturer schema
