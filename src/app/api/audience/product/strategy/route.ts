...
async function productStrategyHandler(req: NextRequest): Promise<Response> {
  ...
  try {
    ...
    if (process.env.NEXT_PUBLIC_USE_MOCK_GPT === 'true' || token === DEMO_TOKEN) {
      ...
      logger.warn('[Product Strategy] Using MOCK response — no real strategy generated (NEXT_PUBLIC_USE_MOCK_GPT or demo token)');
    ...
    if (!creditResult.success) {
      ...
      if (creditResult.error === 'charge_conflict') {
        ...
        logger.error(
          `[Product Strategy] Charge conflict for user ${userId} — nothing charged, output discarded`
        );
        return createSecureResponse(
          {
            success: false,
