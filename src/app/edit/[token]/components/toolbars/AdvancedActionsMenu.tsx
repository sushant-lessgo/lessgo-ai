// app/edit/[token]/components/toolbars/AdvancedActionsMenu.tsx - Enhanced Advanced Actions Dropdown
import React, { forwardRef, useEffect, useRef } from 'react';
import { useAdvancedActionsMenu } from '@/hooks/useAdvancedActionsMenu';

export interface AdvancedActionItem {
  id: string;
  label: string;
  icon: string;
  handler: () => void;
  disabled?: boolean;
  group?: string;
  shortcut?: string;
}

export interface AdvancedActionGroup {
  id: string;
  label?: string;
  actions: AdvancedActionItem[];
}

interface AdvancedActionsMenuProps {
  actions: AdvancedActionItem[];
  triggerElement: HTMLElement;
  onClose: () => void;
  toolbarType: 'section' | 'element' | 'text' | 'form' | 'image';
  isVisible: boolean;
}

export const AdvancedActionsMenu = forwardRef<HTMLDivElement, AdvancedActionsMenuProps>(
  ({ actions, triggerElement, onClose, toolbarType, isVisible }, ref) => {
    const menuRef = useRef<HTMLDivElement>(null);
    
    console.log('🎯 AdvancedActionsMenu rendered with props:', {
      actions: actions?.length || 0,
      hasTriggerElement: !!triggerElement,
      toolbarType,
      isVisible
    });
    
    const {
      position,
      groupedActions,
      updatePosition,
      handleClickOutside,
      handleKeyNavigation,
    } = useAdvancedActionsMenu({
      triggerElement,
      isVisible,
      onClose,
      actions,
      toolbarType,
    });

    // Combine refs
    const combinedRef = (node: HTMLDivElement) => {
      (menuRef as any).current = node;
      if (typeof ref === 'function') {
        ref(node);
      } else if (ref) {
        (ref as any).current = node;
      }
    };

    // Handle click outside
    useEffect(() => {
      if (isVisible) {
        document.addEventListener('mousedown', handleClickOutside);
        document.addEventListener('keydown', handleKeyNavigation);
        
        return () => {
          document.removeEventListener('mousedown', handleClickOutside);
          document.removeEventListener('keydown', handleKeyNavigation);
        };
      }
    }, [isVisible, handleClickOutside, handleKeyNavigation]);

    // Update position when visible
    useEffect(() => {
      if (isVisible && triggerElement) {
        updatePosition();
      }
    }, [isVisible, triggerElement, updatePosition]);

    if (!isVisible) {
      console.log('🎯 AdvancedActionsMenu not visible, returning null');
      return null;
    }
    
    console.log('🎯 AdvancedActionsMenu rendering menu with:', {
      position,
      groupedActionsCount: groupedActions.length,
      groupedActions: groupedActions.map(g => ({ id: g.id, label: g.label, actionCount: g.actions.length }))
    });

    return (
      <div
        ref={combinedRef}
        className="fixed z-60 bg-white border border-gray-200 rounded-lg shadow-lg min-w-[200px] max-w-[280px] py-2"
        style={{
          left: position.x,
          top: position.y,
          transform: position.transform,
        }}
        role="menu"
        aria-orientation="vertical"
        tabIndex={-1}
      >
        {/* Arrow indicator */}
        {position.arrow && (
          <div
            className={`absolute w-2 h-2 bg-white border-gray-200 transform rotate-45 ${
              position.arrow.side === 'top' ? 'border-b border-r -top-1' :
              position.arrow.side === 'bottom' ? 'border-t border-l -bottom-1' :
              position.arrow.side === 'left' ? 'border-r border-b -left-1' :
              'border-l border-t -right-1'
            }`}
            style={{
              left: position.arrow.side === 'left' || position.arrow.side === 'right' ? undefined : position.arrow.offset,
              top: position.arrow.side === 'top' || position.arrow.side === 'bottom' ? undefined : position.arrow.offset,
            }}
          />
        )}

        {/* Menu content */}
        <div className="relative">
          {groupedActions.map((group, groupIndex) => (
            <React.Fragment key={group.id}>
              {/* Group divider */}
              {groupIndex > 0 && (
                <div className="h-px bg-gray-200 my-1" />
              )}
              
              {/* Group label */}
              {group.label && (
                <div className="px-3 py-1 text-xs font-medium text-gray-500 uppercase tracking-wide">
                  {group.label}
                </div>
              )}

              {/* Group actions */}
              {group.actions.map((action) => (
                <button
                  key={action.id}
                  onClick={() => {
                    if (!action.disabled) {
                      action.handler();
                      onClose();
                    }
                  }}
                  disabled={action.disabled}
                  className={`
                    flex items-center w-full px-3 py-2 text-sm text-left transition-colors
                    ${action.disabled 
                      ? 'text-gray-400 cursor-not-allowed' 
                      : 'text-gray-700 hover:bg-gray-50 hover:text-gray-900 focus:bg-gray-50 focus:text-gray-900'
                    }
                    focus:outline-none
                  `}
                  role="menuitem"
                  tabIndex={action.disabled ? -1 : 0}
                >
                  <div className="flex items-center justify-center w-4 h-4 mr-3 flex-shrink-0">
                    <AdvancedActionIcon icon={action.icon} />
                  </div>
                  
                  <span className="flex-1 truncate">{action.label}</span>
                  
                  {action.shortcut && (
                    <span className="ml-2 text-xs text-gray-400 font-mono">
                      {action.shortcut}
                    </span>
                  )}
                </button>
              ))}
            </React.Fragment>
          ))}
        </div>
      </div>
    );
  }
);

AdvancedActionsMenu.displayName = 'AdvancedActionsMenu';

// Enhanced icon component with more comprehensive mapping
function AdvancedActionIcon({ icon }: { icon: string }) {
  const iconMap = {
    // Design & Styling
    'palette': (
      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M7 21a4 4 0 01-4-4V5a2 2 0 012-2h4a2 2 0 012 2v12a4 4 0 01-4 4zM21 5a2 2 0 00-2-2h-4a2 2 0 00-2 2v12a4 4 0 004 4h4a2 2 0 002-2V5z" />
      </svg>
    ),
    'background': (
      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" />
      </svg>
    ),
    'theme': (
      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M7 21a4 4 0 01-4-4V5a2 2 0 012-2h4a2 2 0 012 2v12a4 4 0 01-4 4zM21 5a2 2 0 00-2-2h-4a2 2 0 00-2 2v12a4 4 0 004 4h4a2 2 0 002-2V5z" />
      </svg>
    ),

    // AI & Content
    'refresh': (
      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" />
      </svg>
    ),
    'variations': (
      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 5a1 1 0 011-1h14a1 1 0 011 1v2a1 1 0 01-1 1H5a1 1 0 01-1-1V5zM4 13a1 1 0 011-1h6a1 1 0 011 1v6a1 1 0 01-1 1H5a1 1 0 01-1-1v-6zM16 13a1 1 0 011-1h2a1 1 0 011 1v6a1 1 0 01-1 1h-2a1 1 0 01-1-1v-6z" />
      </svg>
    ),
    'magic': (
      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z" />
      </svg>
    ),

    // Typography
    'font': (
      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M10 20l4-16m4 4l4 4-4 4M6 16l-4-4 4-4" />
      </svg>
    ),
    'line-height': (
      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M8 12h8m-8 6h8m-8-12h8" />
      </svg>
    ),
    'letter-spacing': (
      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M8 9l3 3-3 3m5 0h3" />
      </svg>
    ),
    'text-transform': (
      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M7 16V4m0 0L3 8m4-4l4 4m6 0v12m0 0l4-4m-4 4l-4-4" />
      </svg>
    ),

    // Actions
    'clear': (
      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
      </svg>
    ),
    'copy': (
      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z" />
      </svg>
    ),
    'duplicate': (
      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z" />
      </svg>
    ),

    // Settings & Configuration
    'settings': (
      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z" />
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
      </svg>
    ),
    'advanced': (
      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 6V4m0 2a2 2 0 100 4m0-4a2 2 0 110 4m-6 8a2 2 0 100-4m0 4a2 2 0 100-4m0 4v2m0-6V4m6 6v10m6-2a2 2 0 100-4m0 4a2 2 0 100-4m0 4v2m0-6V4" />
      </svg>
    ),

    // Forms & Validation
    'form': (
      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
      </svg>
    ),
    'validation': (
      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
      </svg>
    ),
    'integrations': (
      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M13.828 10.172a4 4 0 00-5.656 0l-4 4a4 4 0 105.656 5.656l1.102-1.101m-.758-4.899a4 4 0 005.656 0l4-4a4 4 0 00-5.656-5.656l-1.1 1.1" />
      </svg>
    ),

    // Images & Media
    'image': (
      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" />
      </svg>
    ),
    'filter': (
      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M3 4a1 1 0 011-1h16a1 1 0 011 1v2.586a1 1 0 01-.293.707l-6.414 6.414a1 1 0 00-.293.707V17l-4 4v-6.586a1 1 0 00-.293-.707L3.293 7.293A1 1 0 013 6.586V4z" />
      </svg>
    ),
    'crop': (
      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 16V8a4 4 0 014-4h8M4 16l4-4m0 8V8m0 12h8a4 4 0 004-4V8" />
      </svg>
    ),
    'alt-text': (
      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M7 8h10M7 12h4m1 8l-4-4H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-3l-4 4z" />
      </svg>
    ),

    // Performance & Optimization
    'optimize': (
      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M13 10V3L4 14h7v7l9-11h-7z" />
      </svg>
    ),
    'performance': (
      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z" />
      </svg>
    ),
    'lazy': (
      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 3v1m0 16v1m9-9h-1M4 12H3m15.364 6.364l-.707-.707M6.343 6.343l-.707-.707m12.728 0l-.707.707M6.343 17.657l-.707.707" />
      </svg>
    ),

    // Analytics & Insights
    'analytics': (
      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z" />
      </svg>
    ),
    'insights': (
      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z" />
      </svg>
    ),

    // Accessibility
    'accessibility': (
      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
      </svg>
    ),
    'contrast': (
      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <circle cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="2" fill="none"/>
        <path d="M12 2v20" stroke="currentColor" strokeWidth="2"/>
      </svg>
    ),

    // Default fallback
    'default': (
      <div className="w-4 h-4 bg-gray-300 rounded" />
    ),
  };

  return iconMap[icon as keyof typeof iconMap] || iconMap.default;
}