  ...
  return {
    ...
    updateNavItem: (itemId: string, updates: Partial<{ label: string; link: string; sectionId?: string }>) =>
      set((state: EditStore) => {
        ...
        const item = state.navigationConfig.items.find(item => item.id === itemId);
        if (!item) return;
        
        const oldItem = { ...item };
        
        if (updates.label !== undefined) item.label = updates.label;
        if (updates.link !== undefined) item.link = updates.link;
        if (updates.sectionId !== undefined) item.sectionId = updates.sectionId;
        
        // Mark as user-modified if it was auto-generated
        if (item.isAutoGenerated && (updates.label || updates.link)) {
