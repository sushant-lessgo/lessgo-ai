 ...
 *
 * A published page is served on either its `{slug}.lessgo.site` subdomain or, once a
 ...
 */
import { PUBLISH_HOST } from '@/lib/domains/hosts';
...
export function resolveCanonicalURL(opts: {
  ...
  const host = opts.canonicalDomain || `${opts.slug}.${PUBLISH_HOST}`;
  const path =
