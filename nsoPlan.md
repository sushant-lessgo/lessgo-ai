...
- **7.5d TODO (post-merge, ~0.5d):** rename `/api/service/*` → `/api/audience/service/*` (cosmetic; firewall already enforced).
- **Migration NOT yet applied** (dev DB unreachable at edit time) — run `npx prisma migrate deploy` when DB is up. Runtime gates (dogfood copy-diff, edit/publish E2E) deferred to when DB is reachable; all three tickets pass `npm run build`.
